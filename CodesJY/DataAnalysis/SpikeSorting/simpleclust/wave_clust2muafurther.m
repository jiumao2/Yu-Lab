function wave_clust2muafurther(chname)
% not including unsorted units

spkfile = dir(['times_chdat' chname '.mat']);
if isempty(spkfile)
    spkfile = dir(['times_chdat_meansub' chname '.mat']);
end;
...
non_zeros = find(spk.cluster_class(:, 1)~=0);

cluster_ids =spk.cluster_class(non_zeros, 1);
cluster_times = spk.cluster_class(non_zeros, 2);
...
mua.fname = ['times_chdat' chname '.mat'];
mua.opt = [];
mua.header = '';
mua.val2volt = [1 1];
mua.ncontacts = 1;
