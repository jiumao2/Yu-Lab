function plotTurningPreferenceTime(r, trajNotes)
    if nargin<2
        trajNotes = {'RL', 'LR'}; 
    end
    ...
    for k = 1:length(r.Meta)
        t_start_segment(k) = get_t_start_session(r,k);
        t_end_segment(k) = get_t_end_session(r,k);
    end

    for k = 1:length(left_turn_rate_all)
        idx_segment = find(t_start_segment>t(k), 1)-1;
