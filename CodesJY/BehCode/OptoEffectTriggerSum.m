function OptoEffectTriggerSum(bM1Opto, figname, note, stimprofile)
...
for i =1:length(bM1Opto)
    ...
    [~, IndShortNoStim]          = intersect(bM1Opto(i).MedIndex, find(bM1Opto(i).FPs>=indbeg & bM1Opto(i).FPs==FPsl(1) & bM1Opto(i).Stim==0));
    [~, IndShortStim]              = intersect(bM1Opto(i).MedIndex, find(bM1Opto(i).FPs>=indbeg & bM1Opto(i).FPs==FPsl(1) & bM1Opto(i).Stim==1));
    [~, IndLongNoStim]          = intersect(bM1Opto(i).MedIndex, find(bM1Opto(i).FPs>=indbeg & bM1Opto(i).FPs==FPsl(2) & bM1Opto(i).Stim==0));
    [~, IndLongStim]               = intersect(bM1Opto(i).MedIndex,find(bM1Opto(i).FPs>=indbeg & bM1Opto(i).FPs==FPsl(2) & bM1Opto(i).Stim==1));
...
PR_Nostim_Short       =         1000*PR_Nostim_Short - FPsl(1);
PR_Stim_Short           =         1000*PR_Stim_Short - FPsl(1);
PR_Nostim_Long        =         1000*PR_Nostim_Long - FPsl(2);
PR_Stim_Long            =         1000*PR_Stim_Long - FPsl(2);
...
line(1+[-0.1 0.1], [geomean(RTall_Nostim_Short) geomean(RTall_Nostim_Short)], 'color', 'r', 'linewidth', 1)
line([1 1], [RTall_Nostim_Short_se], 'linewidth', 2, 'color', 'r')

line(2+[-0.1 0.1], [geomean(RTall_Stim_Short) geomean(RTall_Stim_Short)], 'color', 'r', 'linewidth', 1)
...
line(3+[-0.1 0.1], [geomean(RTall_Nostim_Long) geomean(RTall_Nostim_Long)], 'color', 'r', 'linewidth', 1) 
line([3 3],RTall_Nostim_Long_se, 'linewidth', 2, 'color', 'r')

line(4+[-0.1 0.1], [geomean(RTall_Stim_Long) geomean(RTall_Stim_Long)], 'color', 'r', 'linewidth', 1)
...
ha2= axes('unit', 'centimeters', 'position', [1.5 6.5 6 3], 'fontsize', 8, 'nextplot', 'add', ...
    'ylim', [0 2000],'ytick', [0:500:3000], 'tickdir', 'out', 'ticklength', [0.015 0.1],...
    'xtick', [1:5], 'xticklabel', {'Short ', 'Short ', 'Long ', 'Long '}, 'xlim', [0 5]);
title(name)

if ~isempty(stimprofile)
    plotshaded([1.5 2.5], [stimprofile(1) stimprofile(1); stimprofile(1)+stimprofile(2) stimprofile(1)+stimprofile(2)],[204 204 255]/255);
    plotshaded(2+[1.5 2.5], [stimprofile(1) stimprofile(1); stimprofile(1)+stimprofile(2) stimprofile(1)+stimprofile(2)],[204 204 255]/255);
end;

plotshaded([.5 2.5], [0 0;  600  600], [200 186 200]/255);
plotshaded([2.5 4.5], [0 0 ; 600  600], [200 186 200]/255);
...
plotshaded([0 1], [0 0; 600 600], [200 186 200]/255);
...
ha2b= axes('unit', 'centimeters', 'position', [12 6.5 2.5 3], 'fontsize', 8, 'nextplot', 'add', ...
    'ylim', [0 2000],'ytick', [0:500:3000],'yticklabel', [],  'tickdir', 'out', 'ticklength', [0.015 0.1],...
    'xtick', [0:0.1:0.5], 'xlim', [0 0.5]);
...
Late_Nostim_short = length(find(PR_Nostim_Short>600))/length(PR_Nostim_Short);
Correct_Nostim_short = 1-Late_Nostim_short;
...
Late_Stim_short = length(find(PR_Stim_Short>600))/length(PR_Stim_Short);
Correct_Stim_short = 1-Late_Stim_short;

hbar1 = bar([6], [Correct_Stim_short],'FaceColor', [55 255 0]/255);
% hbar2 = bar([7], [Premature_Stim_short], 'FaceColor',  [153 51 0]/255);
hbar3 = bar([7], [Late_Stim_short], 'FaceColor', 'r');
...
Late_Nostim_long = length(find(PR_Nostim_Long>600))/length(PR_Nostim_Long);
Correct_Nostim_long = 1-Late_Nostim_long;
...
Late_Stim_long = length(find(PR_Stim_Long>600))/length(PR_Stim_Long);
Correct_Stim_long = 1-Late_Stim_long;
...
ha3= axes('unit', 'centimeters', 'position', [9 11 2.5 3], 'fontsize', 8, 'nextplot', 'add', ...
    'xlim', [0 2000],'xtick', [0:1000:2000], 'ylim', [0 1], 'ytick', [0:.2:1],  'tickdir', 'out', 'ticklength', [0.015 0.1]);
...
plotshaded([0 600], [0 0; 1 1], [200 186 200]/255);
