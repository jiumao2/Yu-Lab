...
if ~isempty(PassiveEvents) && isfield(PassiveEvents, 'Labels') && isfield(PassiveEvents, 'Time')
    for iPassive = 1:length(PassiveEvents.Labels)
        if isempty(PassiveEvents.Time{iPassive})
            continue
        end
        [passivePSTH{end+1}, passiveTs{end+1}, passiveTrialspxmat{end+1}, passiveTspkmat{end+1}] = ...
            Spikes.jpsth(r.Units.SpikeTimes(ku).timings, PassiveEvents.Time{iPassive}, params_passive);
        passivePSTH{end} = smoothdata(passivePSTH{end}, 'gaussian', 5);
        passiveLabels{end+1} = PassiveEvents.Labels{iPassive};
...
%% Reward
col3 = 12;
width = 5;%*sum(RewardTimeDomain)/sum(PressTimeDomain);
...
if num2str(length(t_reward_pokes))>200
    rasterheight = 0.01;
elseif num2str(length(t_reward_pokes))>100
    rasterheight = 0.02;
else
    rasterheight = 0.04;
end
...
k=0;
    ...
    scatter(x_portin, y_portin, 8, 'o', 'filled','MarkerFaceColor', reward_col,  'markerfacealpha', 0.9, 'MarkerEdgeColor','none')
...
%% plot pre-trigger activity vs trial num or time

ha10=axes('unit', 'centimeters', 'position', [col3 yshift_row5new 5 2.5], ...
    'nextplot', 'add', 'xlim', [min(t_triggers/1000) max(t_triggers/1000)]);

ind_pretrigger = find(tspkmat_trigger<0);
spkmat_pretrigger =  trialspxmat_trigger(ind_pretrigger, :);
dur_pretrigger = abs(tspkmat_trigger(ind_pretrigger(1)))/1000; % total time

rate_pretrigger = sum(spkmat_pretrigger, 1)/dur_pretrigger; % spk rate across time
plot(ha10, t_triggers/1000, rate_pretrigger, 'k', 'marker', 'o', 'markersize', 3, 'linestyle', 'none');

% linear regression
Pfit = polyfit(t_triggers/1000,rate_pretrigger,1);
yfit = Pfit(1)*t_triggers/1000+Pfit(2);
plot(t_triggers/1000,yfit,'r:', 'linewidth', 1.5);
...
fig_height = max([yshift_row6new, yshift_row6new+1]);

%% plot trigger-related activity
col4 = 2;
width = 8; %*sum(TriggerTimeDomain)/sum(PressTimeDomain);
...
% for i=1:nFPs
    plot(ts_trigger, psth_trigger, 'color', FP_cols(3, :), 'linewidth', 1.5);
    FRMax = max([FRMax max(psth_trigger)]);
%     disp(FRMax)
% end
...
ntrials_trigger = size(trialspxmat_trigger, 2);


axes('unit', 'centimeters', 'position', [col4 yshift_row2 width ntrials_rewardpoke*rasterheight],...
    'nextplot', 'add',...
    'xlim', [-TriggerTimeDomain(1) TriggerTimeDomain(2)], 'ylim', [-ntrials_rewardpoke 1], 'box', 'on');
yshift_row3 = yshift_row2+ntrials_trigger*rasterheight+0.5;
% Paint the foreperiod
k=0;

    ap_mat = trialspxmat_trigger;
    t_mat = tspkmat_trigger;
...
if ~isempty(passiveLabels)
    ...
    h_passive_psth = gobjects(1, length(passiveLabels));
    ...
    for iPassive = 1:length(passiveLabels)
        ...
        switch passiveLabels{iPassive}
            case 'PassiveTone'
                xlabel('Time from passive tone (ms)')
            case 'PassiveFlash'
                xlabel('Time from passive flash (ms)')
            otherwise
        ...
        switch passiveLabels{iPassive}
            case 'PassiveTone'
                panel_label = [char('F' + iPassive - 1) '. Passive tone'];
            case 'PassiveFlash'
                panel_label = [char('F' + iPassive - 1) '. Passive flash'];
            otherwise
                panel_label = [char('F' + iPassive - 1) '. ' passiveLabels{iPassive}];
        end
    ...
    fig_width = max(fig_width, col_passive + length(passiveLabels)*passive_width + ...
        (length(passiveLabels)-1)*passive_col_gap + 1);
end
...
ha0=axes('unit', 'centimeters', 'position', [col5 yshift_row5new 1.5 1.5], ...
...
ha00= axes('unit', 'centimeters', 'position', [col5+1.5+1 yshift_row5new 2 1.5], 'nextplot', 'add', 'xlim', [-25 25]);
if median(c)>1
...
% if r.Units.SpikeNotes(ku, 5)==1
%     anm_brain = [r.Meta(1).Subject '(Cortex)'];
% elseif r.Units.SpikeNotes(ku, 5)==2
%     anm_brain = [r.Meta(1).Subject '(Striatum)'];
% end
...
%% add location
ha_loc =  axes('unit', 'centimeters', 'position', [col5+1 yshift_row3-1 2 3], 'nextplot', 'add');
...
set(hf, 'position', [2 2 fig_width fig_height])
toc;
