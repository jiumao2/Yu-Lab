...
% PopOut is population PSTH produced by Spikes.DoubleTone.PopulationActivity.m
taskCodes = PopOut.TriggerTypes;
nFPs = length(taskCodes);
n_unit = size(PopOut.Units, 1);
taskIndex = 1; % use the None condition as the sorting reference
...
for i = 1:n_unit
    % press
    pval_pop(1, i)        =      PopOut.PressStat{taskIndex}.StatOut(i).pval;
    tpeaks_pop(1, i)    =      PopOut.PressStat{taskIndex}.StatOut(i).tpeak;
    [~, indpeak]= (min(abs(PopOut.Press{taskIndex}(1, :) - PopOut.PressStat{taskIndex}.StatOut(i).tpeak)));
    spkmax_pop(1, i)  =      PopOut.Press{taskIndex}(i+1, [indpeak]);

    maxFP = max([max(PopOut.Press{taskIndex}(i+1, :)), max(PopOut.Release{taskIndex}(i+1, :)), max(PopOut.Reward{taskIndex}(i+1, :))]);
...
%     line([PopOut.PressStat{FPindx}.StatOut(i).tpeak  PopOut.PressStat{FPindx}.StatOut(i).tpeak], [0 maxFP], 'color', 'm', 'linestyle', '-.', 'linewidth', 1.5)
    ...
    % release
    pval_pop(2, i)        =      PopOut.ReleaseStat{taskIndex}.StatOut(i).pval;
    tpeaks_pop(2, i)    =      PopOut.ReleaseStat{taskIndex}.StatOut(i).tpeak+gap;
    [~, indpeak]= (min(abs(PopOut.Release{taskIndex}(1, :) - PopOut.ReleaseStat{taskIndex}.StatOut(i).tpeak)));
    spkmax_pop(2, i)  =      PopOut.Release{taskIndex}(i+1, [indpeak]);
...
%     title(['Unit# ' num2str(i)])
    % reward
    pval_pop(3, i)        =      PopOut.RewardStat{taskIndex}.StatOut(i).pval;
    tpeaks_pop(3, i)    =      PopOut.RewardStat{taskIndex}.StatOut(i).tpeak+2*gap;
    [~, indpeak]= (min(abs(PopOut.Reward{taskIndex}(1, :) - PopOut.RewardStat{taskIndex}.StatOut(i).tpeak)));
    spkmax_pop(3, i)  =      PopOut.Reward{taskIndex}(i+1, [indpeak]);
