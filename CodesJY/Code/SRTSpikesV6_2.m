function PSTHout = SRTSpikesV6_2(r_all, ind, varargin)
...
% V6_2: merge data from multiple days
...
rIndex_RawChannel_Number = r_all.UnitsCombined(ind,:).rIndex_RawChannel_Number{1};
r = [];

for k = 1:size(rIndex_RawChannel_Number,1)
    r_index = rIndex_RawChannel_Number(k,1);
    channel = rIndex_RawChannel_Number(k,2);
    number = rIndex_RawChannel_Number(k,3);
    temp_path = r_all.r{r_index}.path;
    path_idx = strfind(temp_path,'\');
    relative_path = temp_path(path_idx(end-1)+1:end);
    temp = load(fullfile([relative_path],r_all.r{r_index}.filename));
    r_this = temp.r;

    unit_idx = find(r_this.Units.SpikeNotes(:,1)==channel & r_this.Units.SpikeNotes(:,2)==number);
    
    if isempty(r)
        r = r_this;
        r.Units = rmfield(r.Units,'SpikeTimes');
        r.Units.SpikeTimes(1).timings = r_this.Units.SpikeTimes(unit_idx).timings;
        r.Units.SpikeTimes(1).wave = r_this.Units.SpikeTimes(unit_idx).wave;
        r.Units.SpikeNotes = r.Units.SpikeNotes(unit_idx,:);
    else
        t_next_session = max(r.Units.SpikeTimes(end).timings)+1e6;
        
        r.Behavior.CorrectIndex = [r.Behavior.CorrectIndex;r_this.Behavior.CorrectIndex+sum(r.Behavior.EventMarkers==3)];
        r.Behavior.PrematureIndex = [r.Behavior.PrematureIndex;r_this.Behavior.PrematureIndex+sum(r.Behavior.EventMarkers==3)];
        r.Behavior.LateIndex = [r.Behavior.LateIndex;r_this.Behavior.LateIndex+sum(r.Behavior.EventMarkers==3)];
        r.Behavior.DarkIndex = [r.Behavior.DarkIndex;r_this.Behavior.DarkIndex+sum(r.Behavior.EventMarkers==3)];
        r.Behavior.Foreperiods = [r.Behavior.Foreperiods;r_this.Behavior.Foreperiods];
        r.Behavior.EventTimings = [r.Behavior.EventTimings;r_this.Behavior.EventTimings+t_next_session];
        r.Behavior.EventMarkers = [r.Behavior.EventMarkers;r_this.Behavior.EventMarkers];
        
        r.Units.SpikeTimes(1).timings = [r.Units.SpikeTimes(1).timings,r_this.Units.SpikeTimes(unit_idx).timings+t_next_session];
        r.Units.SpikeTimes(1).wave = [r.Units.SpikeTimes(1).wave;r_this.Units.SpikeTimes(unit_idx).wave];
    end
end
...
hf=27;
figure(27); clf(27)
set(gcf, 'unit', 'centimeters', 'paperpositionmode', 'auto' ,'color', 'w')
...
ha3 =  axes('unit', 'centimeters', 'position', [6.5 6 4*size(trialspxmat{2}, 1)/4000 ntrial2*rasterheight],...
...
ha3 =  axes('unit', 'centimeters', 'position', [12 6 4*size(trialspxmat_premature_press, 1)/4000 ntrial2premature*rasterheight],...
    'nextplot', 'add',  'xticklabel', [],...
...
yshift = 5.5+0.5+ntrial2premature*rasterheight+0.5;
ha3 =  axes('unit', 'centimeters', 'position', [12 yshift 4*size(trialspxmat_premature_press, 1)/4000  ntrial2late*rasterheight],...
    'nextplot', 'add',  'xticklabel', [],...
...
ha4b =  axes('unit', 'centimeters', 'position', [17.5 3.5 4*size(trialspxmat_release{2}, 1)/4000 2], 'nextplot', 'add', 'xlim', [-2000 2500]);
...
yshift = 5.5+0.5;
ha5 =  axes('unit', 'centimeters', 'position', [17.5 yshift 4*size(trialspxmat_release{1}, 1)/4000 ntrial1*rasterheight],...
    'nextplot', 'add',...
...
yshift = yshift;
ha3 =  axes('unit', 'centimeters', 'position', [23 yshift 4*size(trialspxmat_release{2}, 1)/4000 ntrial2*rasterheight],...
    'nextplot', 'add', 'xticklabel', [],...
...
ha6 =  axes('unit', 'centimeters', 'position',  [28.5 yshift 4*size(trialspxmat_premature_release, 1)/4000 ntrial2premature*rasterheight],...
...
ha3c =  axes('unit', 'centimeters', 'position', [28.5 yshift 4*size(trialspxmat_premature_press, 1)/4000  ntrial2late*rasterheight],...
    'nextplot', 'add',  'xticklabel', [],...
...
%% reward
ha7 =  axes('unit', 'centimeters', 'position', [34 1 6 2], 'nextplot', 'add', 'xlim', [-2000 5000]);
...
ha8 =  axes('unit', 'centimeters', 'position', [34 3+0.5 6 ntrial4*rasterheight],...
...
ha7 =  axes('unit', 'centimeters', 'position', [34 yshift 6 2], 'nextplot', 'add', 'xlim', [-2000 5000]);
...
ha8 =  axes('unit', 'centimeters', 'position', [34 yshift 6 ntrial5*rasterheight],...
...
ha7 =  axes('unit', 'centimeters', 'position', [34 yshift 6 2.5], 'nextplot', 'add', 'xlim', [-500 1500]);
...
uicontrol('style', 'text', 'units', 'centimeters', 'position', [ycolumn2(1) yfirstcolumn(2)+0.6 4 0.5],...
    'string', ([r.Meta(1).Subject ' ' r.Meta(1).DateTime(1:11)]), 'BackgroundColor','w', 'fontsize', 10, 'fontweight','bold')

uicontrol('style', 'text', 'units', 'centimeters', 'position', [ycolumn2(1) yfirstcolumn(2) 4 0.5],...
    'string', (['Unit#' num2str(ind) '(Ch' num2str(ch) ')']), 'BackgroundColor','w', 'fontsize', 10, 'fontweight','bold')
%  

% change the height of the figure

FinalHeight = yfirstcolumn(2)+0.6+2;
 
set(27, 'position', [1 1 42 FinalHeight] )
