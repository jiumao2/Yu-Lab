classdef BehaviorClass
    ...
    methods
        ...
        function Plot(obj)
            ...
            set(obj.Fig1, 'unit', 'centimeters', 'position',[2 2 18 18], 'paperpositionmode', 'auto', 'color', 'w')
            ...
            ha0 = axes;
            set(ha0, 'parent', obj.Fig1,'units', 'centimeters', 'position', [xnow ynow, plotsize1],...
            ...
            for k = 1:length(obj.MixedFP)
                ...
                ha1(k) = axes;
                set(ha1(k), 'parent', obj.Fig1,'units', 'centimeters', 'position', [xnow ynow, plotsize1],...
                ...
                % plot press times
                line(ha1(k), [obj.PressTime(obj.Cue == 1 &obj.FP == obj.MixedFP(k)); obj.PressTime(obj.Cue == 1 &obj.FP == obj.MixedFP(k))], [0 250], 'color', 'b')
                
                symbolSize = 5+5*(k-1);
                ind_premature_presses = strcmp(obj.Outcome, 'Premature') & obj.Cue == 1 &obj.FP == obj.MixedFP(k) & obj.Stage == 1;
                scatter(ha1(k), obj.ReleaseTime(ind_premature_presses), ...
                    1000*(obj.ReleaseTime(ind_premature_presses) - obj.PressTime(ind_premature_presses)), ...
                    15, col_perf(2, :), 'o','Markerfacealpha', 0.8, 'linewidth', 1, 'SizeData', symbolSize);
                
                ind_late_presses = strcmp(obj.Outcome, 'Late') & obj.Cue == 1& obj.FP == obj.MixedFP(k) & obj.Stage == 1;
                LateDur =   1000*(obj.ReleaseTime(ind_late_presses) - obj.PressTime(ind_late_presses));
                LateDur(LateDur>2500) = 2499;
                ...
                ind_good_presses = strcmp(obj.Outcome, 'Correct') & obj.Cue == 1 & obj.FP == obj.MixedFP(k) & obj.Stage == 1;
                scatter(ha1(k), obj.ReleaseTime(ind_good_presses), ...
                    1000*(obj.ReleaseTime(ind_good_presses) - obj.PressTime(ind_good_presses)), ...
                    15, col_perf(1, :), 'o', 'Markerfacealpha', 0.8, 'linewidth', 1, 'SizeData', symbolSize);
 
                title(sprintf('%2.1f s', obj.MixedFP(k)/1000))
                
                % Plot histogram
                ha2(k) = axes;
                set(ha2(k), 'parent', obj.Fig1,'units', 'centimeters', 'position', [xnow ynow2, plotsize1],...
                ...
                Press_Duration = obj.ReleaseTime(obj.Cue == 1 &obj.FP == obj.MixedFP(k)) - obj.PressTime(obj.Cue == 1 &obj.FP == obj.MixedFP(k));
                PressCounts = histcounts(Press_Duration, tbins, 'Normalization', 'pdf');
                
                bar(tbin_centers, PressCounts, 'FaceColor', FP_colors{k}, 'EdgeColor', 'none')
                ...
                % Plot reaction time
                ha3(k) = axes;
                set(ha3(k), 'parent', obj.Fig1,'units', 'centimeters', 'position', [xnow ynow3, plotsize1],...
            ...
            % Here goes the probe trials
            ha1probe = axes;
            set(ha1probe, 'parent', obj.Fig1,'units', 'centimeters', 'position', [xnow ynow, plotsize1],...
                'nextplot', 'add', 'ylim', [0 HoldTimeMax], 'xlim', [1 SessionMaxInSecs], 'yscale', 'linear',...
              'ytick', [], 'fontsize', fontsize)
            
            % plot press times
            line(ha1probe, [obj.PressTime(obj.Cue == 0 ); obj.PressTime(obj.Cue == 0)], [0 250], 'color', 'b')
            
            symbolSize = 15;
            ind_probe_presses = obj.Cue == 0;
            scatter(ha1probe, obj.ReleaseTime(ind_probe_presses), ...
                1000*(obj.ReleaseTime(ind_probe_presses) - obj.PressTime(ind_probe_presses)), ...
                25, [.25 .25 .25], '+','Markerfacealpha', 0.8, 'linewidth', 1, 'SizeData', symbolSize);
            
            title(sprintf('%s', 'probe'))
            
            ha2probe = axes;
            set(ha2probe, 'parent', obj.Fig1,'units', 'centimeters', 'position', [xnow ynow2, plotsize1],...
                'nextplot', 'add', 'xlim', [0 HoldTimeMax], 'ylim', density_range, 'yscale', 'linear','ytick', [], 'fontsize', fontsize)            
            % plotting histogram
            Press_Duration = obj.ReleaseTime(obj.Cue == 0) - obj.PressTime(obj.Cue == 0);
            PressCounts = histcounts(Press_Duration, tbins, 'Normalization', 'pdf');
            ...
            ha1dark= axes;
            set(ha1dark, 'parent', obj.Fig1,'units', 'centimeters', 'position', [xnow ynow, plotsize1],...
                'nextplot', 'add', 'ylim', [0 HoldTimeMax], 'xlim', [1 SessionMaxInSecs], 'yscale', 'linear',...
               'ytick', [], 'fontsize', fontsize)
            ...
            ha2dark = axes;
            set(ha2dark, 'parent', obj.Fig1,'units', 'centimeters', 'position', [xnow ynow2, plotsize1],...
                'nextplot', 'add', 'xlim', [0 HoldTimeMax], 'ylim', density_range, 'yscale', 'linear','ytick', [], 'fontsize', fontsize)
            % plotting histogram
            ...
            ha4 = axes;
            set(ha4, 'parent', obj.Fig1, 'units', 'centimeters', ...
            ...
            ha5 = axes; %
            set(ha5,'parent', obj.Fig1,'units', 'centimeters',...
            ...
            % Plot performance for different FPs
            ha6 = axes;
            set(ha6,'parent', obj.Fig1,'units', 'centimeters', 'position', [xnow, ynow4, plotsize2], 'nextplot', 'add', 'ylim', [0 100], ...
                'xlim', [-0.5 2+4*(length(obj.MixedFP)-1)+2], 'xtick', [2:4:2+4*(length(obj.MixedFP)-1)], 'xticklabel', num2cell(obj.MixedFP),...
            ...
            % Plot symbols and their meanings
            hainfo=axes;
            set(hainfo, 'parent', obj.Fig1,'units', 'centimeters', 'position', [xnow ynow4 2 plotsize2(2)], ...
