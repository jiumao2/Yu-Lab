function events = Bpod_Events(sd);
% 2/20/2021
...
for k =1:Ntrials
    ...
    if ~isnan(sd.RawEvents.Trial{k}.States.WaitForRewardEntry(1))
        ...
        events.Reward = [events.Reward t_trial+t_trial+sd.RawEvents.Trial{k}.States.RewardDelivery'];
        events.PokeIn = [events.PokeIn  t_trial+t_trial+sd.RawEvents.Trial{k}.States.Drinking(:, 1)'];
        events.PokeOut = [events.PokeIn  t_trial+t_trial+sd.RawEvents.Trial{k}.States.Drinking(:, 2)'];
    else
        events.PokeIn = [events.PokeIn  t_trial+t_trial+sd.RawEvents.Trial{k}.States.Drinking(:, 1)'];
        events.PokeOut = [events.PokeIn  t_trial+t_trial+sd.RawEvents.Trial{k}.States.Drinking(:, 2)'];
    end;
