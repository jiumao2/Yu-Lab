...
for k = press_idx_start:length(b.PressTime)
    ...
    % label the hand
    if ts.FrameTimes(1) > press_time_this
        continue
    end

    frame_press = findNearestPoint(ts.FrameTimes, press_time_this);
    frames_all = [frame_press+20, frame_press+10, frame_press, frame_press-10, frame_press-20];
    if frame_press > sum(ts.Nframe)
        break
    end

    vid_idx = find(frame_press <= cumsum(ts.Nframe), 1);
    if vid_idx > 1
        frame_press = frame_press - sum(ts.Nframe(1:vid_idx-1));
    ...
    while true
        ...
        elseif out == '?'
            % check the video
            idx_start = max(1, frame_press-20);
            idx_end = min(vid.NumFrames, frame_press+20);
