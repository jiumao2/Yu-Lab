...
for m =1:nFPs
    ...
    for i =1:nFP_i(m)
        ...
        yy2 = [0 1]-k;
        xxrt = irt + fixedFP;
        plotshaded([0 fixedFP],[-k -k; 1-k 1-k], trigger_col);
...
for i =1:size(ap_mat, 2)
    ...
    yy2 = [0 1]-k;
    xxrt = ipredur;
    itrigger = toneTimes(FPs_premature_presses(i));
    plotshaded([0 fixedFP], [-k -k; 1-k 1-k], trigger_col);
    if ~isnan(itrigger)
...
for i =1:size(ap_mat, 2)
    ...
    yy2 = [0 1]-k;
    xxrt = ilatedur;
    itrigger = toneTimes(FPs_late_presses(i));
    plotshaded([0 fixedFP], [-k -k; 1-k 1-k], trigger_col);
    if ~isnan(itrigger)
        patch([0 itrigger itrigger 0], ...
            [-k -k 1-k 1-k], extra_tone_shade_col, 'FaceAlpha', extra_tone_alpha, 'EdgeColor', 'none');
...
for m =1:nFPs
    ...
    for i =1:nFP_i(m)
        ...
        % paint foreperiod
        plotshaded([-irt-fixedFP -irt]-n_start, [-k -k; 1-k 1-k], trigger_col);

        for i_xx = 1:length(xx)
            xx_all = [xx_all, xx(i_xx), xx(i_xx), NaN];
            yy_all = [yy_all, yy1, NaN];
        end
        xxrt_all = [xxrt_all, -irt-fixedFP, -irt-fixedFP, NaN];
        yyrt_all = [yyrt_all, yy2, NaN];
...
for i =1:size(ap_mat, 2)
    ...
    % paint foreperiod
    plotshaded([-ipredur -ipredur+fixedFP], [-k -k; 1-k 1-k], trigger_col);
...
for i =1:size(ap_mat, 2)
    ...
    yy = [0 1]-k;
    % paint foreperiod
    plotshaded([-ilatedur -ilatedur+fixedFP], [-k -k; 1-k 1-k], trigger_col);
