...
for k = release_idx_start:length(b.ReleaseTime)
    ...
    % label the hand
    if ts.FrameTimesSide(1) > release_time_this
        continue
    end

    frame_release = findNearestPoint(ts.FrameTimesSide, release_time_this);
    frames_all = [frame_release+20, frame_release+10, frame_release, frame_release-10, frame_release-20];
    if any(frames_all > sum(ts.NframeSide))
        break
    end

    vid_idx = find(frame_release <= cumsum(ts.NframeSide), 1);
    if vid_idx > 1
        frame_release = frame_release - sum(ts.NframeSide(1:vid_idx-1));
    ...
    while true
        ...
        elseif out == '?'
            % check the video
            idx_start = max(1, frame_release-40);
            idx_end = min(ts.NframeSide(vid_idx), frame_release+40);
