function whiskingepochstim3(T, dataout, contacts, tdelay, vrange, md, whiskamps, th, showspikes,  touchwhisker, whisk_nonwhisk,  wid, badtrials, tosave, type);
...
xstim=intersect(selected_trialnums, allstimtrials);

[vm, aomxstim]=findvmtrials(T, xstim);
allstimonset=[];
falsestim=[];
...
for i=1:length(selected_trialnums)
    ...
    if isempty(touchwhisker)
        for iw=1:length(tids)
            ...
            if ~isempty(touch_iw)
                ...
                last_touch=touch_iw(end, 2);
                touchperiods=[touchperiods; time(first_touch) time(last_touch)];
        ...
        if ~isempty(touchperiods)
            touchperiods=[min(touchperiods(:, 1)) max(touchperiods(:, 2))];
        end;
    else
    ...
    [selected_trialnums(i) t_contact];
        ...
        [vm_tn , ~, tvm, ~, ~, ~, spk_tn]=findvmtrials(T, tri);
        ...
        if any(intersect(tri, allstimtrials))
            ...
            [vm_tri, aom, tvm, fp_tri, ~, ~, spk_tri]=findvmtrials(T, tri);
            aom_all=[aom_all aom];
            ...
            spk_stim_surround=[spk_stim_surround spk_tri(find(aom>1, 1, 'first')-tpre*10:find(aom>1, 1, 'first')+24999)];
            ...
            aom_onoff=zeros(1, length(aom));
            aom_onoff(aom>1)=1;
            aom_onoff=mergegaps(aom_onoff, 500, 2);
            ...
            if n_whisking_epochs_tn>0
                for j=1:n_whisking_epochs_tn
                    ...
                    if  ~isempty(t_contact)
                        if t1<touchperiods(1)
                            t2=min([whisking_epochs_tn(j, 2) t_contact]);
                        elseif t1>touchperiods(1) && t1<=touchperiods(2)
                            t1=touchperiods(2);
                            t2=whisking_epochs_tn(j, 2);
                        else
                            t2=whisking_epochs_tn(j, 2);
                        end;
                    else
                    ...
                    [~, ind_t1]=min(abs(tvm-t1));
                    [~, ind_t2]=min(abs(tvm-t2));
                    
                    newt1t2=zeros(1, length(tvm));
                    newt1t2(ind_t1:ind_t2)=1;
                    newt1t2(aom_onoff==0)=0;
                    
                    t1=tvm(find(newt1t2>0, 1, 'first'));
                    t2=tvm(find(newt1t2>0, 1, 'last'));
            ...
            spk_nostim_surround=[spk_nostim_surround spk_tri];
    ...
    hf_avg=figure;
    set(hf_avg, 'units', 'centimeters', 'position', [4 4 18 14], 'paperpositionmode', 'auto', 'color', 'w')
    ...
    plot(twhisk, mean(whiskamp_stim_all, 2),  'color', stimcol);
    ...
    plot(tvm, mean(removeAP(vm_stim_all,10000, th, 4), 2),  'color', stimcol);
    axis 'auto y'
    ylim=get(ha_avg, 'ylim');
    plot(tvm,  aom_all(:, 1)/2+ylim(1)-2, ':',  'color', stimcol)
...
%     
    ...
    plot(tvm_stim_surround,aom_stim_surround+ylim(1)-4,  'color', stimcol)
    xlabel('Time (s)')
    ylabel('Vm (mV)')
    ...
    plot(thist_surround, spkhist_stim_surround,  'color', stimcol);
    
    axis 'auto y'
    ylim=get(ha_avg, 'ylim');
    plot(tvm_stim_surround,aom_stim_surround/4+ylim(1)-2,  'color', stimcol)
    ...
    plot(tvm_stim_surround, mean(removeAP(vm_stim_surround, 10000, th, 4), 2), 'color', stimcol);
    axis 'auto y'
    ylim=get(ha_avg, 'ylim');
    plot(tvm_stim_surround,aom_stim_surround/4+ylim(1)-2,  'color', stimcol)
    %plot(dataout.poleonset, ylim(2)-2, 'r*', 'markersize', 8)
    ...
    plot(twhisk_stim_surround, mean(whiskamp_stim_surround, 2),   'color', stimcol);
    axis 'auto y'
    xlabel('Time (ms)')
    ylabel('Whisking amp (deg)')
    
    hf1=figure;
    set(gcf, 'units', 'centimeters', 'position', [4 4 28 14],'color', 'w', 'paperpositionmode', 'auto');
    ...
    set(gca, 'xlim', vrange, 'nextplot', 'add');
    bar(x2, n2,  'facecolor', stimcol, 'edgecolor', stimcol)
    ...
    set(gca, 'xlim', vrange, 'nextplot', 'add');
    bar(x4, n4,  'edgecolor', stimcol, 'facecolor', stimcol)
    title(['stim, whisking(4), ' sprintf('%01f',median(vmstim_whisk_avg))])
    ...
    if ~isempty(spkstim_nonwhisk_avg)
        spkstim_nonwhisk_rate=findrate(spkstim_nonwhisk_avg);
    else
        spkstim_nonwhisk_rate=zeros(size(spknostim_nonwhisk_rate));
    end;
    ...
    plot(2, spkstim_nonwhisk_rate(1), 'o', 'color', stimcol)
    line([2 2], [spkstim_nonwhisk_rate(2) spkstim_nonwhisk_rate(3)], 'color', stimcol)
    
    plot(3, spknostim_whisk_rate(1), 'ko')
    line([3 3], [spknostim_whisk_rate(2) spknostim_whisk_rate(3)], 'color', 'k')
    
    plot(4, spkstim_whisk_rate(1), 'o', 'color', stimcol)
    line([4 4], [spkstim_whisk_rate(2) spkstim_whisk_rate(3)], 'color', stimcol)
    
    axis 'auto y'
    xlabel('conditions')
    ylabel('Spk (Hz)')
    posspk=get(gca, 'position');
    set(gca, 'position', posspk, 'xticklabel', {'nw', 'nw', 'wh', 'wh'});
    ...
    plot(stim_whiskamp, stim_vm, 'o', 'markersize', 4, 'color', stimcol);
    ...
    for i=1:length(whiskamps)-1
        ...
        if length(ind_stim)>3
            ...
            line([whiskampsmid(i) whiskampsmid(i)] , [vm_whiskamp_stim_avg(i, 2) vm_whiskamp_stim_avg(i, 3)], 'color', stimcol, 'linewidth', 1);
        else
    ...
    plot(whiskampsmid(~isnan(vm_whiskamp_stim_avg(:, 1))), vm_whiskamp_stim_avg(~isnan(vm_whiskamp_stim_avg(:, 1)), 1), 'o-', 'linewidth', 1, 'color', stimcol);
    ...
    plot(whiskampsmid, spk_whiskamp_stim_avg(:, 1), 'o-', 'linewidth', 1, 'color',stimcol);
    plot([whiskampsmid; whiskampsmid], spk_whiskamp_stim_avg(:, [2 3])', '-', 'linewidth', 1, 'color', stimcol);
