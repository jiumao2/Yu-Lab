...
for ib = 1:length(Pop.IndSort)
    ...
    n_unit = length(IndSort); %size(Pop.Units, 1);
    ...
    TriggerPSTHs = cell(1, length(Pop.Reward));
    TriggerPSTHZs = cell(1, length(Pop.Reward));
    tTriggerPSTHs = cell(1, length(Pop.Reward));
    ...
    for ifp = 1:length(Pop.Reward)
        PSTH_Concatenate{ifp}      =      [TriggerPSTHs{ifp} RewardPokePSTHs{ifp}];
        ...
        % IndRelease{ifp}                    =      IndPress{ifp}(end)+[1:size(ReleasePSTHs{ifp}, 2)];
        IndTrigger{ifp}                    =    [1:size(TriggerPSTHs{ifp}, 2)];
        IndReward{ifp}                     =      IndTrigger{ifp}(end)+[1:size(RewardPokePSTHs{ifp} ,2)];
    end
    ...
    hf=48;
    figure(hf); clf(hf)
    set(gcf, 'unit', 'centimeters', 'position', [2 2 20 20], 'paperpositionmode', 'auto' ,'color', 'w')
    size_factor = 1.5;
    space = 0.1;
    xlevel_start = 1.25;
    % A. Trigger
    ylevel_start = 2;
    map_height = 3;
    tRange = [max(tTriggerPSTHs{ifp}(1), -3000) 2000];
    Width  = size_factor*diff(tRange)/2000;
    for ifp =1:nFP
        ha_Trigger(ifp) = axes('unit', 'centimeters', 'position',...
            [xlevel_start ylevel_start+(map_height+0.5)*(ifp-1)  Width map_height], 'nextplot', 'add',...
            'xlim', tRange, 'xtick', [-3500:500:2000],...
            'ylim', [0.5 n_unit+0.5], 'ydir', 'reverse', 'ticklength', [0.025 0.01], 'XTickLabelRotation', 90);
        if ifp >1
            set(ha_Trigger(ifp), 'xticklabel', [])
        else
            ylabel('Units')
            xlabel('Trigger (ms)')
        end;
        TriggerPSTH_thisFP = PSTH_ConNorm{ifp}(:, IndTrigger{ifp});
        himage1 = imagesc(tTriggerPSTHs{ifp}, [1:n_unit], TriggerPSTH_thisFP, norm_range);
        colormap('Parula')
        ...
        title(sprintf('Ntrials=%2.0d', Pop.Trials{1} ));
    end
    ...
    ylevel_start = 2;
    map_height = 3;
    tRange = [max(tTriggerPSTHs{1}(1), -3000) 2000];
    Width  = size_factor*diff(tRange)/2000;
    for ifp =1:nFP
        ha_Trigger(ifp) = axes('unit', 'centimeters', 'position',...
            [xlevel_start ylevel_start+(map_height+0.5)*(ifp-1)  Width map_height], 'nextplot', 'add',...
            'xlim', tRange, 'xtick', [-3500:500:2000],...
            'ylim', [0.5 n_unit+0.5], 'ydir', 'reverse', 'ticklength', [0.025 0.01], 'XTickLabelRotation', 90);
        colormap(ha_Trigger(ifp), mycolormap);
        if ifp >1
            set(ha_Trigger(ifp), 'xticklabel', [])
        else
            ylabel('Units')
    ...
    if ~isfield(Pop, 'UnitsColumn5')
        str_this = [Pop.Name ' | ', strrep(Pop.Session, '_', '-')];
    else
        str_this = [Pop.Name ' | ', Pop.UnitsColumn5{ib} ' | ' strrep(Pop.Session, '_', '-')];
    end
