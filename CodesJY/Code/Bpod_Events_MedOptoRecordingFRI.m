function events = Bpod_Events_MedOptoRecordingFRI(sd);
% 2/20/2021
...
for k =1:Ntrials
    ...
    if ~isnan(sd.RawEvents.Trial{k}.States.WaitForPokedInFast)  % failed attempt
        events.GoodRelease = [events.GoodRelease t_trial+sd.RawEvents.Trial{k}.States.WaitForMedTTLFast(end)];
        events.GoodPokeIn = [events.GoodPokeIn t_trial+sd.RawEvents.Trial{k}.States.WaitForPokedInFast(2)];
        events.Reward = [events.Reward t_trial+sd.RawEvents.Trial{k}.States.RewardDeliveryFast'];
    elseif ~isnan(sd.RawEvents.Trial{k}.States.WaitForPokedInMedium)  % failed attempt
        events.GoodRelease = [events.GoodRelease t_trial+sd.RawEvents.Trial{k}.States.WaitForMedTTLMedium(end)];
        events.GoodPokeIn = [events.GoodPokeIn t_trial+sd.RawEvents.Trial{k}.States.WaitForPokedInMedium(2)];
        events.Reward = [events.Reward t_trial+sd.RawEvents.Trial{k}.States.RewardDeliveryMedium'];
    elseif ~isnan(sd.RawEvents.Trial{k}.States.WaitForPokedInSlow)  % failed attempt
        events.GoodRelease = [events.GoodRelease t_trial+sd.RawEvents.Trial{k}.States.WaitForMedTTLSlow(end)];
        events.GoodPokeIn = [events.GoodPokeIn t_trial+sd.RawEvents.Trial{k}.States.WaitForPokedInSlow(2)];
        events.Reward = [events.Reward t_trial+sd.RawEvents.Trial{k}.States.RewardDeliverySlow'];
    else % bad press
        press_time = t_trial+sd.RawEvents.Trial{k}.States.WaitForTone(1);
    ...
    if k>1 && isnan(sd.RawEvents.Trial{k-1}.States.WaitForPokedInFast(1)) && isnan(sd.RawEvents.Trial{k-1}.States.WaitForPokedInMedium(1)) && isnan(sd.RawEvents.Trial{k-1}.States.WaitForPokedInSlow(1))
        badpokes = sd.RawEvents.Trial{k}.States.BadPortEntry; % bad poke entries of current trial
        ind_prepress = find(badpokes(:, 1)< sd.RawEvents.Trial{k}.States.WaitForTone(1));
