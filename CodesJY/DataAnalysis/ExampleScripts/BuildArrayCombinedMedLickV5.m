...
for ib=1:length(blocks)
    ...
    EventOut = DIO_Events4(NEV) % create
        ...
        % read bpod events
     load('Lit_MedLick_recording_20210113_114346.mat')
     bpodevents = Bpod_Events_MedLickRecording(SessionData);
     
     % note that there are no poke data recorded in DI
     % have to relie on bpod to add those data point in
     % update poke information in EventOut with bpod events
    EventOut = UpdateDIOMedLick(EventOut, bpodevents);
