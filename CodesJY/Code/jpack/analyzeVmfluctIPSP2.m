function analyzeVmfluctIPSP2(T, contacts, nontrials, prtl, wid, prefph, touchPSPdur, ampth, md, th, plot_ratio, stim, vrange)
if nargin<13
    vrange=[-90 -20];
    if nargin<12
        stim=[];
...
%     end;
    ...
    ivm=sgolayfilt(Vm(:, i), 3, 51);
    
    ivmnoap=ivm; % remove AP
...
%             plot(tvm(indtouchoff{iw}), ivm(indtouchoff{iw}), '^', 'color', iwcolor)
    ...
    set(gca, 'xlim', [0.5 2.5], 'ylim', vrange)
    title('Vm')
    ylabel('mV')
    xlabel('Time (s)')
    axis off
