function PSTHOut = SRTSpikesWithPassive(r, ind, varargin)
...
%% Check the shape of r.Behavior.Foreperiod
...
%% Passive tone and flash events
PassiveEvents.Labels = {};
PassiveEvents.Time = {};
PassiveEvents.PassiveTimeDomain = PassiveTimeDomain;

if ~isempty(t_releases)
    passive_start_time = max(t_releases);
elseif ~isempty(t_presses)
    passive_start_time = max(t_presses);
else
...
for iku =1:length(ku_all)
    ...
    disp('##########################################')
    PSTHOut.PSTH(iku)       = Spikes.SRT.ComputePlotPSTHWithPassive(r, PSTHOut, ku,...
