...
for i =1:length(time_leverpress)
    istart = time_leverpress(i);
    iend = time_leverrelease(i);
    if istart < iend
        time_presses = [time_presses; istart];
        time_releases = [time_releases; iend];
    end;
end;

time_leverpress           =       time_presses;
time_leverrelease        =       time_releases;
...
inter_trial_presses=[];
inter_trial_presses = setdiff(ind_bad_presses, [ind_premature_releases' ind_late_releases]);
% 
% for i = 1:length(ind_bad_presses2)
%     % most recent light on
%     recent_lighton=time_leverlighton(find(time_leverlighton < time_leverpress(ind_bad_presses2(i)), 1, 'last'));
%     % most recent light off
%     recent_lightoff=time_leverlightoff(find(time_leverlightoff < time_leverpress(ind_bad_presses2(i)), 1, 'last'));
%     if ~isempty(recent_lighton) && ~isempty(recent_lightoff) && recent_lightoff > recent_lighton
%         inter_trial_presses=[inter_trial_presses ind_bad_presses2(i)];
%     else
%         pause
%     end;
% end;
