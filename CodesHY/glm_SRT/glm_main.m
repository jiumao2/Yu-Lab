...
clear;
data_path = 'c:/Users/jiumao/Desktop/Russo20210820';
...
xlabel('time(ms)')
xline(0,'--');
subplot(2,2,1);title('Press Weights');ylim([ylim_min,ylim_max]);
subplot(2,2,2);title('Release Weights');ylim([ylim_min,ylim_max]);
subplot(2,2,3);title('Trigger Weights');ylim([ylim_min,ylim_max]);
subplot(2,2,4);title('Holding Weights');ylim([ylim_min,ylim_max]);
% figure;
% ax = axes;
% if is_all_unit_included
...
if is_all_unit_included
    data_psth_long = mean(spikes(:,long_index,number_unit),2)./bin_width*1000;
    data_psth_short = mean(spikes(:,short_index,number_unit),2)./bin_width*1000;
else
