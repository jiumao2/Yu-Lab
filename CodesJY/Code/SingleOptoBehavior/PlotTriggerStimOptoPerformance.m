function PlotTriggerStimOptoPerformance(bmix, manipulation, printname)
...
for i = 1:length(bmix)
    ...
    ind_stim = find(bmix(i).StimTypes == 3);
...
%% Trigger
ha3 = axes('units', 'centimeters', 'position', [6 2 3 4], 'nextplot', 'add','xlim', [0 10], 'ylim', [0 100], 'xtick', [2:5:(size(Performance_Nostim, 1)-1)*5+2.5], 'xticklabel', {'short', 'long'});

for ifp = 1:size(Performance_TriggerStim, 1)
%     for k =1:3
%         hbar(k) =  bar(k+(ifp-1)*5, 100*Performance_TriggerStim(ifp, k+1)/Performance_TriggerStim(ifp, 1));
%         set(hbar(k), 'facecolor',col_perf(k, :), 'edgecolor', 'w', 'BarWidth', 0.9);
%     end;
    
    hbar(1) =  bar(1+(ifp-1)*5, 100*Performance_TriggerStim(ifp, 2)/sum(Performance_TriggerStim(ifp, [2 4])));
    set(hbar(1), 'facecolor',col_perf(1, :), 'edgecolor', 'w', 'BarWidth', 0.9);
    
    hbar(2) =  bar(2+(ifp-1)*5, 100*Performance_TriggerStim(ifp, 4)/sum(Performance_TriggerStim(ifp, [2 4])));
    set(hbar(2), 'facecolor',col_perf(3, :), 'edgecolor', 'w', 'BarWidth', 0.9);
...
%% modified control
ha1 = axes('units', 'centimeters', 'position', [2 2 3 4], 'nextplot', 'add','xlim', [0 10], 'ylim', [0 100], 'xtick', [2:5:(size(Performance_Nostim, 1)-1)*5+2.5], 'xticklabel', {'short', 'long'});

for ifp = 1:size(Performance_Nostim, 1)
    
    hbar(1) =  bar(1+(ifp-1)*5, 100*Performance_Nostim(ifp, 2)/sum(Performance_Nostim(ifp, [2 4])));
    set(hbar(1), 'facecolor',col_perf(1, :), 'edgecolor', 'w', 'BarWidth', 0.9);
    
    hbar(2) =  bar(2+(ifp-1)*5, 100*Performance_Nostim(ifp, 4)/sum(Performance_Nostim(ifp, [2 4])));
    set(hbar(2), 'facecolor',col_perf(3, :), 'edgecolor', 'w', 'BarWidth', 0.9);

end;

title('Control')
ylabel('Performance (modified)')
...
sprintf('RT short FP control %2.2f ms, trigger-stim %2.2f ms', median(RT_NoStim_Short), median(RT_TriggerStim_Short))

RTShortdata = {RT_NoStim_Short, RT_TriggerStim_Short};
RTShortdataCat = [zeros(1, length(RT_NoStim_Short)), ones(1, length(RT_TriggerStim_Short))];
 
plotSpread(RTShortdata, 'categoryIdx', RTShortdataCat,  'categoryMarkers',{'.','.'},...
    'categoryColors',{[0.8 0.8 0.8],[0 184 255]/255}, 'categoryLabels', {'NoStim', 'Stim(trigger)'},'spreadWidth', 0.8, 'binWidth', 0.2)

line([1 1], RTNoStimCI95_ShortFP, 'color', 'k', 'linewidth', 1)
plot(1, median(RT_NoStim_Short), 'o', 'markersize', 4, 'linewidth', 1, 'color', 'k')

line([2 2], RT_TriggerStimCI95_ShortFP, 'color', 'k', 'linewidth', 1)
plot(2, median(RT_TriggerStimCI95_ShortFP), 'o', 'markersize', 4, 'linewidth', 1, 'color', 'k')
...
RT_Short = [RT_NoStim_Short RT_TriggerStim_Short];
Group_Short = [repmat({'nostim'}, 1, length(RT_NoStim_Short)) repmat({'trigger'}, 1, length(RT_TriggerStim_Short))];
...
RTLongdata = {RT_NoStim_Long, RT_TriggerStim_Long};
RTLongdataCat = [zeros(1, length(RT_NoStim_Long)), ones(1, length(RT_TriggerStim_Long))];

sprintf('RT long FP control %2.2f ms, trigger-stim %2.2f ms', median(RT_NoStim_Long), median(RT_TriggerStim_Long))
...
line([2 2], RT_TriggerStimCI95_LongFP, 'color', 'k', 'linewidth', 1)
plot(2, median(RT_TriggerStim_Long), 'o', 'markersize', 4, 'linewidth', 1, 'color', 'k')
% 
...
RT_Long = [RT_NoStim_Long RT_TriggerStim_Long];
Group_Long = [repmat({'nostim'}, 1, length(RT_NoStim_Long)) repmat({'trigger'}, 1, length(RT_TriggerStim_Long))];
...
axis off
hainfo = axes('units', 'centimeters', 'position', [2.5 14 4 2], 'nextplot', 'add','xlim', [0 10], 'ylim', [0 10],'xtick', [], 'ytick', []);
axis off
...
PrDur_NoStim_Short = PrDur_NoStim_Short(PrDur_NoStim_Short>=FPUnique(1))
...
PrDurShortdata = {PrDur_NoStim_Short, PrDur_TriggerStim_Short};
PrDurShortdataCat = [zeros(1, length(PrDur_NoStim_Short)), ones(1, length(PrDur_TriggerStim_Short))];
...
% add stimulus timing
plotshaded([0 0.2], [mean(StimDelay)+FPUnique(1)  mean(StimDelay)+FPUnique(1) ;mean(StimDelay)+mean(StimDur)+FPUnique(1)  mean(StimDelay)+mean(StimDur)+FPUnique(1)], 'b')
...
% density
ha7ksd = axes('units', 'centimeters', 'position', [16.5 8 2.5 4], 'nextplot', 'add','xlim', [0 3000], 'ylim', [-0.1 1],'xtick', [0:1000:3000], 'xticklabel', {'0', '1', '2', '3'},'ytick', [0:0.25:1]);
timebins = [0:50:3000];
plotshaded([FPUnique(1) FPUnique(1)+600], [0 0; 1 1], [175 255 175]/255)
plotshaded([mean(StimDelay)+FPUnique(1) mean(StimDelay)+mean(StimDur)+FPUnique(1)], [0 0; -0.1 -0.1], 'b')

[f_ksNoStim] = ksdensity(PrDur_NoStim_Short, timebins,'Support','positive',...
	'Function','cdf','Bandwidth',0.025); 
[f_ksStim] = ksdensity([ PrDur_TriggerStim_Short], timebins,'Support','positive',...
	'Function','cdf','Bandwidth',0.025); 
...
PrDur_NoStim_Long = PrDur_NoStim_Long(PrDur_NoStim_Long>=FPUnique(2));
...
PrDurLongdata = {PrDur_NoStim_Long,  PrDur_TriggerStim_Long};
PrDurLongdataCat = [zeros(1, length(PrDur_NoStim_Long)), ones(1, length(PrDur_TriggerStim_Long))];
 
plotSpread(PrDurLongdata, 'categoryIdx', PrDurLongdataCat,  'categoryMarkers',{'.','.'},...
    'categoryColors',{[0.8 0.8 0.8],[0 184 255]/255}, 'categoryLabels', {'NoStim', 'Stim(press)'},'spreadWidth', 0.8, 'binWidth', 0.2)
 
plotshaded([0 0.2], [mean(StimDelay)+FPUnique(2)  mean(StimDelay)+FPUnique(2) ;mean(StimDelay)+mean(StimDur)+FPUnique(2)  mean(StimDelay)+mean(StimDur)+FPUnique(2)], 'b')
...
% 
% density
ha8ksd = axes('units', 'centimeters', 'position', [26 8 2.5 4], 'nextplot', 'add','xlim', [0 3000], 'ylim', [-0.10 1],'xtick', [0:1000:3000], 'xticklabel', {'0', '1', '2', '3'},'ytick', [0:0.25:1]);
timebins = [0:50:3000];
...
[f_ksNoStim] = ksdensity(PrDur_NoStim_Long, timebins,'Support','positive',...
	'Function','cdf','Bandwidth',0.025); 
[f_ksStim] = ksdensity([PrDur_TriggerStim_Long], timebins,'Support','positive',...
	'Function','cdf','Bandwidth',0.025); 
plot(timebins, f_ksNoStim, 'color',  'k', 'linewidth', 1)
plot(timebins, f_ksStim, 'color',  [0 184 255]/255, 'linewidth', 1)

plotshaded([mean(StimDelay)+FPUnique(2) mean(StimDelay)+mean(StimDur)+FPUnique(2)], [0 0; -0.1 -0.1], 'b')
...
savename = ['OptoTriggerStim_' Dataout.ANM printname];
