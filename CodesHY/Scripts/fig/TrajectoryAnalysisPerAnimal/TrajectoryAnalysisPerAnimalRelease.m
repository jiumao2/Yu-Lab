...
% vid_side_traj2_filename = 'D:\Ephys\ANMs\Eli\Sessions\20210923_video\VideoFrames_side\RawVideo\Press059.avi';
% animal_name = 'Eli';
% unit_num = 1;
% event = 'release';
% t_pre = -500;
...
vid_top = VideoReader(vid_top_filename);
bg_top = vid_top.read(-r.VideoInfos_top(1).t_pre/10);
vid_side = VideoReader(vid_side_traj1_filename);
bg_side_traj1 = vid_side.read(-r.VideoInfos_top(1).t_pre/10);
vid_side = VideoReader(vid_side_traj2_filename);
...
xlim(ax_yt1,[t_pre_press,t_post_press])
ylim(ax_yt1,[200,900])
...
xlim(ax_yt2,[t_pre_press,t_post_press])
ylim(ax_yt2,[200,900])
% ax_yt2.XAxis.Visible = 'off';

annotation(h,'textbox',[0.3,0.485,0.4,0.05],'String','Time from lift (ms)','EdgeColor','none','FontSize',8.25,'HorizontalAlignment','center','VerticalAlignment','middle');
...
annotation(h,'textbox',[0.3,0,0.4,0.05],'String','Time from release (ms)','EdgeColor','none','FontSize',8.25,'HorizontalAlignment','center','VerticalAlignment','middle');
...
% traj topview
interval_top = 10;
frame_diff = r.VideoInfos_top(1).total_frames-r.VideoInfos_side(1).total_frames;

for k = 1:length(index_cat1)
    x1 = r.VideoInfos_top(ind_correct(index_cat1(k))).Tracking.Coordinates_x{ind_bodypart_top}(frame_diff+1:end);
    y1 = r.VideoInfos_top(ind_correct(index_cat1(k))).Tracking.Coordinates_y{ind_bodypart_top}(frame_diff+1:end);
    p1 = r.VideoInfos_top(ind_correct(index_cat1(k))).Tracking.Coordinates_p{ind_bodypart_top}(frame_diff+1:end);
...
for k = 1:length(index_cat2)
    x2 = r.VideoInfos_top(ind_correct(index_cat2(k))).Tracking.Coordinates_x{ind_bodypart_top}(frame_diff+1:end);
    y2 = r.VideoInfos_top(ind_correct(index_cat2(k))).Tracking.Coordinates_y{ind_bodypart_top}(frame_diff+1:end);
    p2 = r.VideoInfos_top(ind_correct(index_cat2(k))).Tracking.Coordinates_p{ind_bodypart_top}(frame_diff+1:end);
...
% sideview
frame_num_press = (t_post_press-r.VideoInfos_side(1).t_pre)/10+1;
interval_side = 1;

for k = 1:length(index_cat1)
%     p1 = r.VideoInfos_side(ind_correct(index_cat1(k))).Tracking.Coordinates_p{ind_bodypart_side};
    n_start = r.VideoInfos_side(ind_correct(index_cat1(k))).LiftStartFrameNum;
    if isnan(n_start)
        continue
    end
...
%     p2 = r.VideoInfos_side(ind_correct(index_cat2(k))).Tracking.Coordinates_p{ind_bodypart_side};
    n_start = r.VideoInfos_side(ind_correct(index_cat2(k))).LiftStartFrameNum;
    if isnan(n_start)
        continue
    end
%     for j = frame_num_press:-1:1
...
%         end
%     end
    x1 = r.VideoInfos_side(ind_correct(index_cat1(k))).Tracking.Coordinates_x{ind_bodypart_side}(n_start-n_frames_before:frame_num_press);
    y1 = r.VideoInfos_side(ind_correct(index_cat1(k))).Tracking.Coordinates_y{ind_bodypart_side}(n_start-n_frames_before:frame_num_press);
    x1 = x1(1:interval_side:end);
    y1 = y1(1:interval_side:end);
    plot(ax_yt1,r.VideoInfos_side(ind_correct(index_cat1(k))).VideoFrameTime(n_start-n_frames_before:frame_num_press)-r.VideoInfos_side(ind_correct(index_cat1(k))).VideoFrameTime(n_start),y1,'Color',colors_name{1});
end
for k = rnd_cat2
    p2 = r.VideoInfos_side(ind_correct(index_cat2(k))).Tracking.Coordinates_p{ind_bodypart_side};
    n_start = r.VideoInfos_side(ind_correct(index_cat2(k))).LiftStartFrameNum;
...
%         end
%     end
    x2 = r.VideoInfos_side(ind_correct(index_cat2(k))).Tracking.Coordinates_x{ind_bodypart_side}(n_start-n_frames_before:frame_num_press);
    y2 = r.VideoInfos_side(ind_correct(index_cat2(k))).Tracking.Coordinates_y{ind_bodypart_side}(n_start-n_frames_before:frame_num_press);
    x2 = x2(1:interval_side:end);
    y2 = y2(1:interval_side:end);
    plot(ax_yt2,r.VideoInfos_side(ind_correct(index_cat2(k))).VideoFrameTime(n_start-n_frames_before:frame_num_press)-r.VideoInfos_side(ind_correct(index_cat2(k))).VideoFrameTime(n_start),y2,'Color',colors_name{2});
end
...
for k = 1:length(rnd_cat1)
    ...
    t_release = r.VideoInfos_top(ind_this).Time+r.VideoInfos_top(ind_this).Foreperiod+r.VideoInfos_top(ind_this).ReactTime;
    spk_time = r.Units.SpikeTimes(unit_num).timings(t_release+t_pre<=r.Units.SpikeTimes(unit_num).timings & t_release+t_post>=r.Units.SpikeTimes(unit_num).timings);
    spk_time = spk_time - t_release;
    if ~isempty(spk_time)
...
temp1 = [r.VideoInfos_top(ind_correct(index_cat1))];
temp2 = [r.VideoInfos_top(ind_correct(index_cat2))];
trigtimes1 = [temp1.Time]+[temp1.Foreperiod]+[temp1.ReactTime];
trigtimes2 = [temp2.Time]+[temp2.Foreperiod]+[temp2.ReactTime];
...
h_annotation_press_text = annotation(h,'textbox',...
    ...
    'FitBoxToText','off');
set(h_annotation_press_text,'Position',[-0.1,13.8,0.5,0.5]);
