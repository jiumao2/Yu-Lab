function PlotMixedOptoPerformance(bmix, manipulation, filename)
...
hf19 = figure(19); clf,
set(gcf, 'name', 'OptoStimMix', 'units', 'centimeters', 'position', [2 1 34 19], 'paperpositionmode', 'auto');
...
% short FP
ha0 = axes('units', 'centimeters', 'position', [15 3 5 3], 'nextplot', 'add','xlim', [0 length(bmix)+1], 'ylim', [0-5 100], 'ytick', [20:20:100],'ygrid', 'on', 'xtick', [1:length(bmix)], 'xticklabel', SessionNames, 'XTickLabelRotation', 90, 'fontsize', 8);
...
ha0note = axes('units', 'centimeters', 'position', [20.2 3 1 3], 'nextplot', 'add','xlim', [0 10], 'ylim', [0 10], 'xtick', [ ], 'xticklabel', [], 'XTickLabelRotation', 90, 'fontsize', 8);
...
ha00 = axes('units', 'centimeters', 'position', [24.5 3 5 3], 'nextplot', 'add','xlim', [0 length(bmix)+1], 'ylim', [0-5 100], 'ytick', [20:20:100],'ygrid', 'on',  'xtick', [1:length(bmix)], 'xticklabel', SessionNames, 'XTickLabelRotation', 90, 'fontsize', 8);
...
%% Press
ha3 = axes('units', 'centimeters', 'position', [10 8 3 4], 'nextplot', 'add','xlim', [0 10], 'ylim', [0 100], 'xtick', [2:5:(size(Performance_Nostim, 1)-1)*5+2.5], 'xticklabel', {'short', 'long'});
...
if sum(Performance_ApproachStim(:))>0
...
%% plot reaction time
ha6 = axes('units', 'centimeters', 'position', [15 14 5 4], 'nextplot', 'add','xlim', [0 4.5], 'ylim', [100 850],'yscale', 'linear','ytick', [200:200:600], 'xtick', [1 2], 'xticklabel', {'Control', 'Press', 'Trigger'});
...
sprintf('RT short FP control %2.2f ms, press-stim %2.2f ms, trigger-stim %2.2f ms', median(RT_NoStim_Short), median(RT_PressStim_Short), median(RT_TriggerStim_Short))

RTShortdata = {RT_NoStim_Short, RT_PressStim_Short, RT_TriggerStim_Short};
RTShortdataCat = [zeros(1, length(RT_NoStim_Short)), ones(1, length(RT_PressStim_Short)), 2*ones(1, length(RT_TriggerStim_Short))];
 
plotSpread(RTShortdata, 'categoryIdx', RTShortdataCat,  'categoryMarkers',{'.','.', '.'},...
    'categoryColors',{[0.8 0.8 0.8],[0 184 255]/255, [0 84 255]/255}, 'categoryLabels', {'NoStim', 'Stim(press)', 'Stim(trigger)'},'spreadWidth', 0.8, 'binWidth', 0.2)
...
RT_Short = [RT_NoStim_Short RT_PressStim_Short RT_TriggerStim_Short];
Group_Short = [repmat({'nostim'}, 1, length(RT_NoStim_Short)) repmat({'press'}, 1, length(RT_PressStim_Short))  repmat({'trigger'}, 1, length(RT_TriggerStim_Short))];
...
ha6stat = axes('units', 'centimeters', 'position', [20.1 14 2 4], 'nextplot', 'add','xlim', [0 10], 'ylim', [0 11],'xtick', [], 'ytick', []);
...
text(1, 2, sprintf('KW test '), 'fontsize', 10)
text(1, 1, sprintf('p=%2.5f', p_ShortFP), 'fontsize', 10)

axis off

%% plot data of long FP
ha6 = axes('units', 'centimeters', 'position', [24.5 14 5 4], 'nextplot', 'add','xlim', [0 4.5], 'ylim', [100 850],'yscale', 'linear','ytick', [200:200:600], 'xtick', [1 2], 'xticklabel', {'Control', 'Press', 'Trigger'});
...
RTLongdata = {RT_NoStim_Long, RT_PressStim_Long, RT_TriggerStim_Long};
RTLongdataCat = [zeros(1, length(RT_NoStim_Long)), ones(1, length(RT_PressStim_Long)), 2*ones(1, length(RT_TriggerStim_Long))];

sprintf('RT long FP control %2.2f ms, press-stim %2.2f ms, trigger-stim %2.2f ms', median(RT_NoStim_Long), median(RT_PressStim_Long), median(RT_TriggerStim_Long))
...
RT_Long = [RT_NoStim_Long RT_PressStim_Long RT_TriggerStim_Long];
Group_Long = [repmat({'nostim'}, 1, length(RT_NoStim_Long)) repmat({'press'}, 1, length(RT_PressStim_Long))  repmat({'trigger'}, 1, length(RT_TriggerStim_Long))];
...
ha6stat = axes('units', 'centimeters', 'position', [29.6 14 2 4], 'nextplot', 'add','xlim', [0 10], 'ylim', [0 11],'xtick', [], 'ytick', []);
...
text(1, 8, sprintf('p=%2.4f', c_short(2, 6)), 'fontsize', 10)
text(1, 2, sprintf('KW test '), 'fontsize', 10)
text(1, 1, sprintf('p=%2.5f', p_LongFP), 'fontsize', 10)

axis off


hainfo = axes('units', 'centimeters', 'position', [9.5 2 4 2], 'nextplot', 'add','xlim', [0 10], 'ylim', [0 10],'xtick', [], 'ytick', []);
axis off
...
%% plot press duration
ha7 = axes('units', 'centimeters', 'position', [15 8 5 4], 'nextplot', 'add','xlim', [0 5], 'ylim', [0 4000],'yscale', 'linear','ytick', [0:1000:5000], 'xtick', [1 2 3 4], 'xticklabel', {'Control','App', 'Press', 'Trigger'});
% plot short

plotshaded([0.5 4.5], [bmix(1).FPUnique(1) bmix(1).FPUnique(1); bmix(1).FPUnique(1)+600 bmix(1).FPUnique(1)+600],[175 255 175]/255)

PrDur_NoStim_Short = PrDur_NoStim{1}; 
PrDur_PressStim_Short = PrDur_PressStim{1};
PrDur_TriggerStim_Short = PrDur_TriggerStim{1};

if ~isempty(PrDur_ApproachStimLatency{1})
    ...
    PrDurShortdata = {PrDur_NoStim_Short, PrDur_AppStim_Short, PrDur_PressStim_Short, PrDur_TriggerStim_Short};
    PrDurShortdataCat = [zeros(1, length(PrDur_NoStim_Short)), ones(1, length(PrDur_AppStim_Short)), 2*ones(1, length(PrDur_PressStim_Short)), 3*ones(1, length(PrDur_TriggerStim_Short))];
    
    plotSpread(PrDurShortdata, 'categoryIdx', PrDurShortdataCat,  'categoryMarkers',{'.','.', '.', '.'},...
        'categoryColors',{[0.8 0.8 0.8],[0 184 255]/255,[0 184 255]/255, [0 84 255]/255}, 'categoryLabels', {'NoStim', 'Stim(app)', 'Stim(press)', 'Stim(trigger)'},'spreadWidth', 0.8, 'binWidth', 0.2)
    
else
    PrDurShortdata = {PrDur_NoStim_Short, PrDur_PressStim_Short, PrDur_TriggerStim_Short};
    PrDurShortdataCat = [zeros(1, length(PrDur_NoStim_Short)), ones(1, length(PrDur_PressStim_Short)), 2*ones(1, length(PrDur_TriggerStim_Short))];
    
    plotSpread(PrDurShortdata, 'categoryIdx', PrDurShortdataCat,  'categoryMarkers',{'.','.', '.'},...
        'categoryColors',{[0.8 0.8 0.8],[0 184 255]/255, [0 84 255]/255}, 'categoryLabels', {'NoStim', 'Stim(press)', 'Stim(trigger)'},'spreadWidth', 0.8, 'binWidth', 0.2)
    set(ha7, 'xlim', [0 4], 'ylim', [0 4000],'yscale', 'linear','ytick', [0:1000:5000], 'xtick', [1 2 3], 'xticklabel', {'Control','Press', 'Trigger'})
end;
...
% density
ha7ksd = axes('units', 'centimeters', 'position', [20.5 8 2.5 4], 'nextplot', 'add','xlim', [0 4000], 'ylim', [0 1],'xtick', [0:1000:5000], 'xticklabel', {'0', '1', '2', '3'},'ytick', [0:0.25:1]);
timebins = [0:50:5000];
plotshaded([bmix(1).FPUnique(1) bmix(1).FPUnique(1)+600 ], [0 0; 1 1], [175 255 175]/255)

[f_ksNoStim] = ksdensity(PrDur_NoStim_Short, timebins,'Support','positive',...
	'Function','cdf','Bandwidth',0.025); 

if ~isempty(PrDur_ApproachStimLatency{1})
    [f_ksStim] = ksdensity([PrDur_AppStim_Short PrDur_PressStim_Short], timebins,'Support','positive',...
        'Function','cdf','Bandwidth',0.025);
else
...
ha8 = axes('units', 'centimeters', 'position', [24.5 8 5 4], 'nextplot', 'add','xlim', [0 4.5], 'ylim', [0 4000],'yscale', 'linear','ytick', [0:1000:5000], 'xtick', [1 2], 'xticklabel', {'Control', 'App', 'Press', 'Trigger'});
% plot long
plotshaded([0.5 4.5], [bmix(1).FPUnique(2) bmix(1).FPUnique(2); bmix(1).FPUnique(2)+600 bmix(1).FPUnique(2)+600],[175 255 175]/255)

PrDur_NoStim_Long = PrDur_NoStim{2}; 
PrDur_PressStim_Long = PrDur_PressStim{2};
PrDur_TriggerStim_Long = PrDur_TriggerStim{2};

if ~isempty(PrDur_ApproachStimLatency{2})
    ...
    PrDurLongdata = {PrDur_NoStim_Long, PrDur_AppStim_Long, PrDur_PressStim_Long, PrDur_TriggerStim_Long};
    PrDurLongdataCat = [zeros(1, length(PrDur_NoStim_Long)), ones(1, length(PrDur_AppStim_Long)) 2*ones(1, length(PrDur_PressStim_Long)), 3*ones(1, length(PrDur_TriggerStim_Long))];
    
    plotSpread(PrDurLongdata, 'categoryIdx', PrDurLongdataCat,  'categoryMarkers',{'.','.', '.', '.'},...
        'categoryColors',{[0.8 0.8 0.8],[0 184 255]/255, [0 184 255]/255,  [0 84 255]/255}, 'categoryLabels', {'NoStim', 'Stim(app)', 'Stim(press)', 'Stim(trigger)'},'spreadWidth', 0.8, 'binWidth', 0.2)
    
else
    PrDurLongdata = {PrDur_NoStim_Long, PrDur_PressStim_Long, PrDur_TriggerStim_Long};
    PrDurLongdataCat = [zeros(1, length(PrDur_NoStim_Long)) ones(1, length(PrDur_PressStim_Long)), 2*ones(1, length(PrDur_TriggerStim_Long))];
    
    plotSpread(PrDurLongdata, 'categoryIdx', PrDurLongdataCat,  'categoryMarkers',{'.','.', '.'},...
        'categoryColors',{[0.8 0.8 0.8],[0 184 255]/255,   [0 84 255]/255}, 'categoryLabels', {'NoStim', 'Stim(press)', 'Stim(trigger)'},'spreadWidth', 0.8, 'binWidth', 0.2)
    
    set(ha8, 'xlim', [0 4], 'ylim', [0 4000],'yscale', 'linear','ytick', [0:1000:4000], 'xtick', [1 2 3], 'xticklabel', {'Control','Press', 'Trigger'})
...
% 
% density
ha8ksd = axes('units', 'centimeters', 'position', [30 8 2.5 4], 'nextplot', 'add','xlim', [0 4000], 'ylim', [0 1],'xtick', [0:1000:5000], 'xticklabel', {'0', '1', '2', '3'},'ytick', [0:0.25:1]);
timebins = [0:50:5000];
plotshaded([bmix(1).FPUnique(2) bmix(1).FPUnique(2)+600 ], [0 0; 1 1], [175 255 175]/255)

[f_ksNoStim] = ksdensity(PrDur_NoStim_Long, timebins,'Support','positive',...
    'Function','cdf','Bandwidth',0.025);
if ~isempty(PrDur_ApproachStimLatency{2})
    [f_ksStim] = ksdensity([PrDur_AppStim_Long PrDur_PressStim_Long], timebins,'Support','positive',...
        'Function','cdf','Bandwidth',0.025);
else
...
savename = ['OptoMixed' filename '_' Dataout.ANM];
