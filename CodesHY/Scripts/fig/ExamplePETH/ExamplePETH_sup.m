clear
save_filename_pdf = './ExamplePETH_sup.pdf';
save_filename_png = './ExamplePETH_sup.png';
...
data_path_unit = {...
    ...
    'D:\Ephys\ANMs\Russo\Sessions\20210907\RTarrayAll.mat', 13,... % holding
    };
data_path_lift = {'D:\Ephys\ANMs\Russo\Sessions\20210820_video\RTarrayAll.mat', 3};
data_path_press = {'D:\Ephys\ANMs\Urey\Videos\20211124_video\RTarrayAll.mat', 13};
data_path_holding = {'D:\Ephys\ANMs\Russo\Sessions\20210907\RTarrayAll.mat', 13};
data_path_release = {'D:\Ephys\ANMs\Urey\Videos\20211124_video\RTarrayAll.mat', 18};
ntrial_raster = 40;
...
xlim(ax_raster_lift1,[t_pre_lift,t_post_lift])
...
xlim(ax_raster_lift2,[t_pre_lift,t_post_lift])
...
xlim(ax_raster_press1,[t_pre_press,t_post_press])
...
xlim(ax_raster_press2,[t_pre_press,t_post_press])
...
xlim(ax_raster_holding1,[t_pre_holding,t_post_holding]);
...
xlim(ax_raster_holding2,[t_pre_holding,t_post_holding]);
...
xlim(ax_raster_release1,[t_pre_release,t_post_release]);
title(ax_raster_release1,'D','FontSize',10)
...
xlim(ax_raster_release2,[t_pre_release,t_post_release]);
...
t_lift_long_idx = find(FPs == 1500 & ~isnan(t_lift) & strcmp({r.VideoInfos_side.Hand},'Left') & correct_index);
t_lift_long = t_lift(t_lift_long_idx);
t_lift_short_idx = find(FPs == 750 & ~isnan(t_lift) & strcmp({r.VideoInfos_side.Hand},'Left') & correct_index);
...
for k = 1:length(rnd)
    spk_time = r.Units.SpikeTimes(unit_num).timings(t_lift_long(rnd(k))+t_pre_lift<=r.Units.SpikeTimes(unit_num).timings & t_lift_long(rnd(k))+t_post_lift>=r.Units.SpikeTimes(unit_num).timings);
...
rnd = rnd(sort_idx);
for k = 1:length(rnd)
    spk_time = r.Units.SpikeTimes(unit_num).timings(t_lift_short(rnd(k))+t_pre_lift<=r.Units.SpikeTimes(unit_num).timings & t_lift_short(rnd(k))+t_post_lift>=r.Units.SpikeTimes(unit_num).timings);
...
t_lift = [r.VideoInfos_side.LiftStartTime];
t_press_long_idx = find(FPs == 1500 & ~isnan(t_lift) & strcmp({r.VideoInfos_side.Hand},'Left') & correct_index);
t_press_long = t_press(t_press_long_idx);
t_press_short_idx = find(FPs == 750 & ~isnan(t_lift) & strcmp({r.VideoInfos_side.Hand},'Left') & correct_index);
...
for k = 1:length(rnd)
    spk_time = r.Units.SpikeTimes(unit_num).timings(t_press_long(rnd(k))+t_pre_press<=r.Units.SpikeTimes(unit_num).timings & t_press_long(rnd(k))+t_post_press>=r.Units.SpikeTimes(unit_num).timings);
...
for k = 1:length(rnd)
    spk_time = r.Units.SpikeTimes(unit_num).timings(t_press_short(rnd(k))+t_pre_press<=r.Units.SpikeTimes(unit_num).timings & t_press_short(rnd(k))+t_post_press>=r.Units.SpikeTimes(unit_num).timings);
...
params.pre = -t_pre_press;
params.post = t_post_press;
params.binwidth = binwidth_PSTH;
...
for k = 1:length(rnd)
    spk_time = r.Units.SpikeTimes(unit_num).timings(t_press_long(rnd(k))+t_pre_holding<=r.Units.SpikeTimes(unit_num).timings & t_press_long(rnd(k))+t_post_holding>=r.Units.SpikeTimes(unit_num).timings);
    spk_time = spk_time - t_press_long(rnd(k));
    if ~isempty(spk_time)
        numspikes=length(spk_time);
        xx=ones(3*numspikes,1)*nan;
...
for k = 1:length(rnd)
    spk_time = r.Units.SpikeTimes(unit_num).timings(t_press_short(rnd(k))+t_pre_holding<=r.Units.SpikeTimes(unit_num).timings & t_press_short(rnd(k))+t_post_holding>=r.Units.SpikeTimes(unit_num).timings);
    spk_time = spk_time - t_press_short(rnd(k));
    if ~isempty(spk_time)
        numspikes=length(spk_time);
        xx=ones(3*numspikes,1)*nan;
...
params.pre = -t_pre_holding;
params.post = t_post_holding;
params.binwidth = binwidth_PSTH;
[psth1, tpsth1] = jpsth(r.Units.SpikeTimes(unit_num).timings, t_press_long, params);
[psth2, tpsth2] = jpsth(r.Units.SpikeTimes(unit_num).timings, t_press_short, params);
psth1 = smoothdata (psth1, 'gaussian', 5);
...
for k = 1:length(rnd)
    spk_time = r.Units.SpikeTimes(unit_num).timings(t_release_long(rnd(k))+t_pre_release<=r.Units.SpikeTimes(unit_num).timings & t_release_long(rnd(k))+t_post_release>=r.Units.SpikeTimes(unit_num).timings);
    ...
    if ~isempty(spk_time)
        ...
        plot(ax_raster_release1,xx,yy,'-','Color',colors_name{1});  
    end
    t_press_this = t_press_long(rnd(k))-t_release_long(rnd(k));
    plot(ax_raster_release1,[0,0],[-0.5+k,0.5+k],'-','Color',color_release,'LineWidth',linewidth_event);
    plot(ax_raster_release1,[t_press_this,t_press_this],[-0.5+k,0.5+k],'-','Color',color_press,'LineWidth',linewidth_event);
    plot(ax_raster_release1,[t_press_this+1500,t_press_this+1500],[-0.5+k,0.5+k],'-','Color',color_trigger,'LineWidth',linewidth_event);
end
...
for k = 1:length(rnd)
    spk_time = r.Units.SpikeTimes(unit_num).timings(t_release_short(rnd(k))+t_pre_release<=r.Units.SpikeTimes(unit_num).timings & t_release_short(rnd(k))+t_post_release>=r.Units.SpikeTimes(unit_num).timings);
    ...
    if ~isempty(spk_time)
        ...
        plot(ax_raster_release2,xx,yy,'-','Color',colors_name{2});  
    end
    t_press_this = t_press_short(rnd(k))-t_release_short(rnd(k));
    plot(ax_raster_release2,[0,0],[-0.5+k,0.5+k],'-','Color',color_release,'LineWidth',linewidth_event);
    plot(ax_raster_release2,[t_press_this,t_press_this],[-0.5+k,0.5+k],'-','Color',color_press,'LineWidth',linewidth_event);
    plot(ax_raster_release2,[t_press_this+750,t_press_this+750],[-0.5+k,0.5+k],'-','Color',color_trigger,'LineWidth',linewidth_event);
end
params.pre = -t_pre_release;
params.post = t_post_release;
params.binwidth = binwidth_PSTH;
