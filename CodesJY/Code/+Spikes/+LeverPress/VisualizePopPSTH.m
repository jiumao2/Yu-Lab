...
for ib = 1:length(Pop.IndSort)
    ...
    for ifp = 1:length(Pop.Press)
        ...
        %
        % [~, indmax]=max(PSTH_Concatenate{ifp}');
        % [~,IndSortnew] = sort(indmax);
        % PSTH_Concatenate{ifp} = PSTH_Concatenate{ifp}(IndSortnew,:);
    ...
    ylevel_start = 2;
    map_height = 3;
    tRange = [max(tPressPSTHs{ifp}(1),-2500) 2000];
    ...
    for ifp =1:nFP
        ...
        title(sprintf('Ntrials=%2.0d', Pop.Trials ));
    end
    ...
    ylevel_start = 2;
    map_height = 3;
    tRange = [-600 min(1000, tReleasePSTHs{1}(end))];
    Width  = size_factor*diff(tRange)/2000;
    for ifp =1:nFP
    ...
    ylevel_start = 2;
    map_height = 3;
    tRange = [max(tPressPSTHs{1}(1),-2500) 2000];
    Width  = size_factor*diff(tRange)/2000;
    for ifp =1:nFP
        ha_press(ifp) = axes('unit', 'centimeters', 'position',...
            [xlevel_start ylevel_start+(map_height+0.5)*(ifp-1)  Width map_height], 'nextplot', 'add',...
