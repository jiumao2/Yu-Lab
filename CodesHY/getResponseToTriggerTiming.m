function [t, unit_info] = getResponseToTriggerTiming(r, unit, varargin)
...
RT_range = [0, 600]; % only consider trials where RT is in this range to avoid anticipatory response and the interference of release-related response
...
unit_info.FP = mode(r.Behavior.Foreperiods);
...
unit_info.cue = r.Behavior.CueIndex(correct_idx, 2);
...
idx_good_trial = find(((idx_good_rt & unit_info.cue==1) | (unit_info.RTs > RT_range(1) & unit_info.cue==0)) ...
    & unit_info.FPs==unit_info.FP & idx_trigger_type);
...
unit_info.cue = unit_info.cue(idx_good_trial);
unit_info.idx_cued = find(unit_info.cue == 1);
unit_info.idx_uncued = find(unit_info.cue == 0);

if length(unit_info.idx_cued) < min_n_trial || length(unit_info.idx_uncued) < min_n_trial
    warning(['Correct trials (n = ', num2str(length(unit_info.idx_cued)), ') less than ', num2str(min_n_trial), '!']);
%     t = NaN;
%     unit_info = [];
%     return
end
...
for k = 1:n_bins
    ...
    trigger_times = unit_info.press_times+unit_info.FP;

    trial_type = zeros(length(trigger_times),1);
    trial_type(unit_info.idx_cued) = 1; % trial_type = 1 if FP = 750 in this trial
