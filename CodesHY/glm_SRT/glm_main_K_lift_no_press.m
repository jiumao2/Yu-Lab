...
% 1kernel -> best and save -> 2kernel ->...-> all kernels -> plot
clear;
...
Kernels.names = {'release','trigger','holding','lift_highest'};
Kernels.ridge_lambda = {1,1,1,1};
Kernels.l2_lambda = {0,0,0,0};
Kernels.n_par = {release_kernel_post-release_kernel_pre+1, ...
...
n_par = release_kernel_post-release_kernel_pre+1 ...
    +trigger_kernel_post-trigger_kernel_pre+1 ...
    +holding_kernel_post-holding_kernel_pre+1 ... 
    +lift_highest_kernel_post-lift_highest_kernel_pre+1;
...
for k = 1:size(bins,2)
    for j = 1:size(bins,1)
        ...
        lift_highest_stim(j,k) = sum(t_bins(j+1,k)>=lift_highest_time_correct & lift_highest_time_correct>t_bins(j,k));
        for i = 1:len_unit
            if is_all_unit_included
...
if show_figure
    subplot(224);
    plot(ttk,ttk*0, 'k--', ttk, wmap(2:end),'linewidth', 2);
...
if show_figure
subplot(224);
    h = plot(ttk,ttk*0, 'k--', ttk, wmap(2:end),...
        ttk, wmap(2:end),'linewidth', 1);
    xlabel('time before spike (s)'); axis tight;
...
w_lift_highest = wmap(p:p+lift_highest_kernel_post-lift_highest_kernel_pre)./bin_width*1000;
p = p+lift_highest_kernel_post-lift_highest_kernel_pre+1;
...
xline(0,'--');
subplot(2,2,4)
plot((lift_highest_kernel_pre:lift_highest_kernel_post)*bin_width,flip(w_lift_highest));
xlabel('time(ms)')
xline(0,'--');
subplot(2,2,1);title('Release Weights');ylim([ylim_min,ylim_max]);
subplot(2,2,2);title('Trigger Weights');ylim([ylim_min,ylim_max]);
subplot(2,2,3);title('Holding Weights');ylim([ylim_min,ylim_max]);
subplot(2,2,4);title('Lift (highest point) Weights');ylim([ylim_min,ylim_max]);% figure;
% ax = axes;
% if is_all_unit_included
...
t_y = linspace(t_pre-min_kernel_pre*bin_width,t_post-max_kernel_post*bin_width,length(y_psth_cue));
