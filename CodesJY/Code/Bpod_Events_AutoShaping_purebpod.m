function events = Bpod_Events_AutoShaping_purebpod(sd);
% 2/20/2021
...
for k =1:Ntrials
    ...
    if ~isnan(sd.RawEvents.Trial{k}.States.RewardDelivery(1))  % failed attempt
        % events.GoodRelease = [events.GoodRelease t_trial+sd.RawEvents.Trial{k}.States.WaitForMedTTL(end)];
        events.GoodPokeIn = [events.GoodPokeIn t_trial+sd.RawEvents.Trial{k}.States.WaitForPoke(2)];
        events.Reward = [events.Reward t_trial+sd.RawEvents.Trial{k}.States.RewardDelivery'];
        events.GoodWait = [events.GoodWait t_trial+sd.RawEvents.Trial{k}.States.WaitForLED'];
        % events.WaitForPoke = [events.WaitForPoke t_trial+sd.RawEvents.Trial{k}.States.WaitForPoke'];
    elseif k>1 && isnan(sd.RawEvents.Trial{k-1}.States.InvalidPoke(1))
        badpokes = sd.RawEvents.Trial{k}.States.InvalidPoke; % bad poke entries of current trial

            events.BadPokeIn =      [events.BadPokeIn; t_trial+badpokes(:, 1)];
            events.BadPokeOut =    [events.BadPokeOut; t_trial+badpokes(:, 2)];
            events.BadWait = [events.BadWait t_trial+sd.RawEvents.Trial{k}.States.WaitForLED'];
    end
          
    % InvalidPoke
    % if k>1 && isnan(sd.RawEvents.Trial{k-1}.States.InvalidPoke(1))
        % badpokes = sd.RawEvents.Trial{k}.States.InvalidPoke; % bad poke entries of current trial
        % 
        %     events.BadPokeIn =      [events.BadPokeIn; t_trial+badpokes(:, 1)];
        %     events.BadPokeOut =    [events.BadPokeOut; t_trial+badpokes(:, 2)];
...
% bad_pokein=[];
% bad_pokeout=[];
% for i=1:length(events.BadWait)
% 
%     if i<length(events.BadWait)-1
%         t_badpress = events.BadWait(i);
%         t_badpressnext = events.BadWait(i+1);
% 
