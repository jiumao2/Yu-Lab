...
for ifp = 1:nFP
    ...
    thisPSTH                            =       Pop.TriggerZ{ifp}(2:end, :);
    TriggerPSTHZs{ifp}               =       thisPSTH(IndSort, :);
    thisPSTH                            =       Pop.Trigger{ifp}(2:end, :);
    TriggerPSTH{ifp}                 =       thisPSTH(IndSort, :);
    tTriggerPSTHs{ifp}              =       Pop.TriggerZ{ifp}(1, :);

    thisPSTH                            =       Pop.RewardZ{ifp}(2:end, :);
    RewardPokePSTHZs{ifp}               =       thisPSTH(IndSort, :);
    thisPSTH                            =       Pop.Reward{ifp}(2:end, :);
...
for ifp = 1:length(Pop.Press)
    PSTH_Concatenate{ifp}      =      [PressPSTHs{ifp} ReleasePSTHs{ifp} RewardPokePSTHs{ifp}];
...
SignificantMod(Pop.IndUnmodulated) = 0;
Unit_Sorted      =     Pop.IndSort';

for i =1:length(Pop.IndSort)
...
aGoodName = ['PopOut_Flash_' Pop.Name '_' Pop.Session '.csv'];
...
tosavename= fullfile(thisFolder, ['PopulationActivity_Flash_' Pop.Name '_' strrep(Pop.Session, '_', '')]);
disp('########## making figure ########## ')
tic
