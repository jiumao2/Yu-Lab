...
if ~isfield(r.Units, 'SpikeNotesColumn5')
    str_this = [r.BehaviorClass.Date ' | ' r.BehaviorClass.Protocol];
else
    anm_brain = [r.Meta(1).Subject, '(', r.Units.SpikeNotesColumn5{r.Units.SpikeNotes(ku, 5)},')'];
    str_this =  [anm_brain ' '  r.BehaviorClass.Date ' | ' r.BehaviorClass.Protocol];
end
...
% Add information  13.5 3+0.5 6 ntrial4*rasterheight
uicontrol('Style','text','Units','centimeters','Position',[col3-0.5 yshift_row6new 3.5 0.5],...
...
uicontrol('Style','text','Units','centimeters','Position',[col5 yshift_row7 5 1.5],...
    'string', 'F. Spike waveform and autocorrelation', ...
    'FontName','Dejavu Sans', 'fontweight', 'bold','fontsize', 10,'BackgroundColor',[1 1 1],'ForegroundColor', 'k', ...
    'HorizontalAlignment','Left');
fig_height = max([fig_height, yshift_row7+2]);
%% add location
ha_loc =  axes('unit', 'centimeters', 'position', [col3+3 yshift_row6new+1 2 3], 'nextplot', 'add');
...
fig_height = max([fig_height, yshift_row6new+5]);
%%
