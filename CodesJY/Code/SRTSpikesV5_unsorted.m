...
%     {'PokeOffset'}
    ...
    plot(t_correctsorted{1}+FP_short, 2, 'r^');


    plot(t_correctsorted{2},  5, 'ko');
    hold on
    plot(t_trigger_long_correct, 5.2, 'r*');
    plot(t_correctsorted{2}+FP_long, 5, 'r^');
    set(gca, 'ylim', [0 6])
    ...
    for i =1:size(trialspxmat_release{1}, 2)
        ...
        if  isempty(find(isnan(apmat(:, i))))
            ...
            line([xxrt xxrt]-FP_short, yy, 'color', 'g', 'linewidth', 1.5) % this is the press time       
            k = k+1;
        end;
    ...
    for i =1:size(trialspxmat_release{2}, 2)
        ...
        if  isempty(find(isnan(apmat(:, i))))
            ...
            line([xxrt xxrt], yy, 'color', 'm', 'linewidth', 1.5)
            line([xxrt xxrt]-FP_long, yy, 'color', 'g', 'linewidth', 1.5) % this is the press time
            k = k+1;
        end;
