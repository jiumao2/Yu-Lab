function turningAnalysis()
load RTarrayAll.mat
ts = load('timestamps.mat');
ts = ts.ts;
...
if isfield(r.VideoInfos_top, 'Trajectory')
...
% 1:correct; 2:premature; 3:late; 4:dark
    if strcmpi(r.Behavior.Outcome(idx_press), 'Correct')
        out = 1;
    elseif strcmpi(r.Behavior.Outcome(idx_press), 'Premature')
        out = 2;
    elseif strcmpi(r.Behavior.Outcome(idx_press), 'Late')
        out = 3;
    elseif strcmpi(r.Behavior.Outcome(idx_press), 'Dark')
        out = 4;
    elseif strcmpi(r.Behavior.Outcome(idx_press), 'NaN')
        out = -1;
    else
