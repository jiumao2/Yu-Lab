...
%% Check the shape of r.Behavior.Foreperiod and trigger type
press_times = r.Behavior.EventTimings(r.Behavior.EventMarkers == find(strcmpi(r.Behavior.Labels, 'LeverPress')));
...
if size(triggerTypes, 1) ~= size(press_times, 1)
    triggerTypes = triggerTypes';
end

if isfield(rb, 'TriggerTypeLabels') && ~isempty(rb.TriggerTypeLabels)
    taskLabels = cellstr(string(rb.TriggerTypeLabels(:)))';
else
    taskLabels = {'None', 'Tone500', 'Tone750', 'Tone1000'};
end
...
for iTask = 1:nFPs
    if startsWith(taskLabels{iTask}, 'Tone', 'IgnoreCase', true)
        toneTimes(iTask) = str2double(extractAfter(string(taskLabels{iTask}), "Tone"));
    end
end
...
% sort reward according to trigger type
t_rewards_sorted = cell(1, nFPs);
move_time_sorted = cell(1, nFPs);
for ifp =1:nFPs
    ind = find(FP_rewards == taskCodes(ifp));
...
t_reward_pokes              =          cell(1, nFPs);
dt                                     =          []; % poke leading to reward
for ifp = 1:nFPs
    for i=1:length(t_rewards{ifp})
        t_portin_this = t_portin(t_portin >t_rewards{ifp}(i)-1000 & t_portin<t_rewards{ifp}(i)+100);
...
t_rewards_prime = cell2mat(t_rewards');
...
t_triggers = rb.EventTimings(rb.EventMarkers == ind_triggers); % trigger time in ms. 
...
t_triggers_FPs = cell(1, nFPs);
RT_triggers_FPs = cell(1, nFPs);

for i =1:nFPs
    t_triggers_FPs{i}           =            t_triggers(strcmp(triggers_types, 'Correct') & triggers_FPs == taskCodes(i));
    RT_triggers_FPs{i}      =            triggers_RTs(strcmp(triggers_types, 'Correct') & triggers_FPs == taskCodes(i));
...
PSTHOut.TaskTypes.Codes                            =     taskCodes;
...
PSTHOut.Presses.FP                                    =     {taskCodes, FPs_premature_presses, FPs_late_presses};
...
PSTHOut.Releases.Labels                             =     [taskLabels, 'Premature', 'Late'];
PSTHOut.Releases.Time                                =     [t_correct_releases_sorted, t_premature_releases, t_late_releases];
PSTHOut.Releases.FP                                    =     {taskCodes, FPs_premature_releases, FPs_late_releases};
...
PSTHOut.Triggers.Labels                                  =       {'TriggerTime_DifferentTriggerTypes' 'TriggerTime_Late'}; 
PSTHOut.Triggers.Time                                     =       [t_triggers_FPs, t_triggers_late];
