function PressOut = ExtractMovementParamsSD(filename)
% e.g., filename = 'Denny_20210430_Press.xlsx';
% This is to deal with SD rat's chaotic paw movement.
% 8/18/2021 Jianing Yu
...
for i =1:length(FlexOnset)
    if isnumeric(FlexOnset{i})
        Press.FlexOnset(i) = -abs(FlexOnset{i});
    else
        FlexOn_i =  str2num(FlexOnset{i});
        if ~isempty(FlexOn_i) && isnumeric(FlexOn_i(1)) % two numbers
            Press.FlexOnset(i) = -abs(FlexOn_i(1));
        else
...
for i =1:length(TouchOnset)
    if isnumeric(TouchOnset{i})
        Press.TouchOnset(i) = -abs(TouchOnset{i});
    else
        TouchOn_i = str2num(TouchOnset{i});
        if ~isempty(TouchOn_i) && isnumeric(TouchOn_i(1))
            Press.TouchOnset(i) = -abs(TouchOn_i(1));
        else
...
for i =1:length(ReleaseOnset)
    if isnumeric(ReleaseOnset{i})
        ...
        ReleaseOn_i = str2num(ReleaseOnset{i});
        if ~isempty(ReleaseOn_i) && isnumeric(ReleaseOn_i(1))
            Press.ReleaseOnset(i) = ReleaseOn_i(1);
        else
...
for i =1:length(ReleasePaw)
    ReleasePawi = ReleasePaw{i};
    switch ReleasePawi(1)
        case 'L'
...
hbar31 = bar([9], [ReleasePaw_L ]);
set(hbar31, 'Facecolor', 'c')
hbar32 = bar([10], [ ReleasePaw_R ]);
set(hbar32, 'Facecolor', 'b')
hbar33 = bar([11], [ ReleasePaw_B]);
