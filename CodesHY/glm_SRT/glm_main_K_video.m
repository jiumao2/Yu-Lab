...
VidObj = VideoWriter('visualization');
VidObj.FrameRate = 10;
open(VidObj)
    
Kernels.names = {'press','release','trigger','holding'};
...
%% Form training set
training_set_length = (press_kernel_post-press_kernel_pre+release_kernel_post-release_kernel_pre+trigger_kernel_post);
%     -spike_kernel_pre*len_unit);
...
xlabel('time(ms)')
xline(0,'--');
subplot(2,2,1);title('Press');ylabel('Hz');
subplot(2,2,2);title('Release');ylabel('Hz');
subplot(2,2,3);title('Trigger');ylabel('Hz');
subplot(2,2,4);title('Holding');ylabel('Hz');
% figure;
% ax = axes;
% if is_all_unit_included
...
subplot(2,1,1); title('Cue')
subplot(2,1,2); title('Uncue')

%% Save the output
Kernels.wmap = wmap;
saveas(h_psth,[save_path,'/PSTH.png'])
saveas(h_kernel,[save_path,'/kernel.png'])
saveas(h_prediction,[save_path,'/prediction.png'])
save([save_path,'/Kernels.mat'],'Kernels')
