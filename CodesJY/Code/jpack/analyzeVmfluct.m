...
%     end;
    ...
    Vm_clust=sgolayfilt(removeAP(Vm_clust, 10000, 10, 4), 3, 51);
        
    if isempty(stim)
        baseline(i)=prctile(Vm_clust(:), prtl);
    else
        baseline(i)=mean(prctile(Vm_clust(tvm>=stimbeg & tvm<=stimend, :), prtl));
    end;
    
    ivm=sgolayfilt(Vm(:, i), 3, 21);
    
    ivmnoap=sgolayfilt(removeAP(Vm(:, i), 10000, 6, 4), 3, 21); % remove AP
    spk=spikespy(Vm(:, i), 10000, th, 4);
...
%         nonwhiskamp=[];
    ...
    hf(i)=figure;
    set(hf(i),'visible','off', 'units', 'centimeter', 'position', [1 2 15 18],'paperpositionmode', 'auto', 'color', 'w')
    
    havm=subplot(3, 1, 3);
    plot(tvm, ivm, 'k'); hold on
    plot(tvm, aomi/4+min(ivm)-2, 'b');
    plot(tvm(find(spk)), ivm(find(spk)), 'r*'); % spikes
...
%             plot(tvm(indtouchoff{iw}), ivm(indtouchoff{iw}), '^', 'color', iwcolor)
    ...
    set(gca, 'xlim', [0.5 4.5], 'ylim', vrange)
    title('Vm')
    ylabel('mV')
    xlabel('Time (s)')
    axis off
    ...
    for iph=1:length(indnearph0)
        ...
        if ~isempty(find((diffph')>2 & iwhisk_params.twhisk<iwhisk_params.twhisk(indnearph0(iph)))) && ~isempty(find((diffph')<-2 & iwhisk_params.twhisk>iwhisk_params.twhisk(indnearph0(iph))))
    ...
    for iph=1:length(indnearph0)
        ...
        if ~any(diff(iphase)<0) && max(iphase)>3 && min(iphase)<-3
...
if isempty(stim)
    ...
    corr_whiskampVm.Spk_nw=nonwhiskSpk;
    ...
    corr_whiskampVmstim.Spk_nw=nonwhiskSpk;
    save corr_whiskampVmstim corr_whiskampVmstim
end;

if isempty(stim)
    cd C:\Work\Projects\BehavingVm\Data\Groupdata\noise_group
    saveas(hfsum, ['noise_distribution', T.cellNum], 'fig')
    export_fig (hfsum, ['noise_distribution', T.cellNum],'-pdf', '-tiff')
end
