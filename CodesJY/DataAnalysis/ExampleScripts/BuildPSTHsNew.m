...
figure(25); clf(25)
set(gcf, 'unit', 'centimeters', 'position',[2 2 22 1.75*n_unit+2.5], 'paperpositionmode', 'auto' ,'color', 'w')
...
for ku = 1:n_unit
    ...
    % plot spike waveform
    ha0=subplot(n_unit, n_events+3, (ku-1)*(n_events+3)+1)
    ...
    ha00=subplot(n_unit, n_events+3, (ku-1)*(n_events+3)+2)
    if max(c)>1
    ...
    for i = 1:n_events
        ...
        if i==1
            ha(i)=subplot(n_unit, n_events+3, (ku-1)*(n_events+3)+i+[2 3])
        else
            ha(i)=subplot(n_unit, n_events+3, (ku-1)*(n_events+3)+i+3)
        end;
...
figure(30); clf
set(gcf, 'unit', 'centimeters', 'position',[2 2 15 n_unit*1+6], 'paperpositionmode', 'auto' , 'color', 'w')
...
ha2                                                 = axes;
set(ha2, 'units', 'centimeters', 'position', [2 5.2 12 n_unit], 'xlim', get(ha1, 'xlim'), 'ylim', [0.5 size(r.Units.SpikeNotes , 1)+0.5], 'ytick', [1:1:40],'ydir', 'reverse', 'nextplot', 'add', 'fontsize', 8);
linkaxes([ha1, ha2], 'x')
...
title([r.Meta(1).Subject r.Meta(1).DateTime])

print (gcf,'-dpng', ['Events_Spikes_All' ])
% saveas(gcf, 'Events_Spikes_All, 'fig')

%% MultiNeuronCrossCorrelation;
...
for ku = 1:n_unit
    ...
    for ju = 1:n_unit
        ...
        if ku==1
            ...
            if size(wave2plot, 2)>64
                wave2plot=wave2plot(:, [1:64])
            end;
    ...
    set(ha0, 'nextplot', 'add', 'xtick', [], 'ytick', [],  'xlim', [-20 20])
    [c, lags] = xcorr(kutime2, jutime2, 20); % max lag 100 ms
    
    plot(lags, c, 'k')
    
    if diff(get(ha0, 'ylim'))<500
        set(ha0, 'ylim', [0 500]);
    end;
