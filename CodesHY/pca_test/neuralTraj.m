...
%        John Cunningham  jcunnin@stanford.edu
  ...
  % Obtain binned spike counts
    if sum(dat(1).spikes>2)<=0
        seq  = getSeq(dat, binWidth, extraOpts{:});
    else
        for k = 1:length(dat)
            seq(k).y = dat(k).spikes;
            seq(k).T = size(dat(k).spikes,2);
            seq(k).trialId = k;
        end
    end
  ...
  N    = length(seq);
  fdiv = floor(linspace(1, N, numFolds+1));
  % original: fdiv = floor(linspace(1, N+1, numFolds+1))
  % maybe a bug. (modified on 2021.8.7)
