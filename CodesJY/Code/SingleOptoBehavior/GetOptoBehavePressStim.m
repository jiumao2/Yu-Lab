...
StimTypeLabels     =        {'NoStim', 'PressStim'};
...
%          RewardDeliveryHigh: [NaN NaN]
    ...
    if  isfield(sd.RawEvents.Trial{j}.States, 'WaitForMedTTLStim') && ~isnan(sd.RawEvents.Trial{j}.States.WaitForMedTTLStim(1))  % if this state exists, it is a stim trial following approach
        StimTypes(j)        =       2; % 3. stimulation triggered by lever press
...
%     
%     Approach time is not recorded in states. but it is actually recorded
%     in events. 
    ...
    if ~isnan(sd.RawEvents.Trial{j}.States.WaitForPress(1))
        if sd.RawEvents.Trial{j}.States.WaitForPress(1, 2) == sd.RawEvents.Trial{j}.States.Masking(1, 1)
