function analyzeVmfluct(T, contacts, nontrials, prtl, wid, prefph, touchPSPdur, ampth, md, th, plot_ratio, stim, vrange)
if nargin<13
    vrange=[-90 -20];
    if nargin<12
        stim=[];
...
for i=1:length(trialnums)
    ...
    if any(find(aomi>2))
        stimbeg=tvm(find(aomi>2, 1, 'first'));
    ...
    if ~isempty(nw_periods)
        for inw=1:size(nw_periods, 1)
            if isempty(stim)
                ...
                if any((nw_periods(inw, 1)-touch_blank(1)).*(nw_periods(inw, 2)-touch_blank(2))<0)...
                        ||mean(iwhisk_params.phase(iwhisk_params.twhisk>=nw_periods(inw, 1) & iwhisk_params.twhisk<=nw_periods(inw, 2)))>1.5...
                        ||any((nw_periods(inw, 1)-licktime).*(nw_periods(inw, 2)-licktime)<0) || nw_periods(inw, 2)<stimbeg || nw_periods(inw, 2)>stimend || isempty(stimbeg)
                    nw_exclude=[nw_exclude inw];
                end;
            end;
        end;
