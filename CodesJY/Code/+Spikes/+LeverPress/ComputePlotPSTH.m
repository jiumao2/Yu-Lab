...
t_presses = PSTHOut.Presses.Time;
...
psth_presses_correct= psth_presses_all;
ts_press=ts_press_all;
trialspxmat_press=trialspxmat_press_all;
tspkmat_press=tspkmat_press_all;
t_presses_correct = t_correct_presses_all;
...
% end
PSTH.PressesLabels = {'PSTH', 'tPSTH', 'SpikeMat', 'tSpikeMat', 'tEvents'};
% PSTH.Presses = PressesAll;
% Release PSTH
params_releases.pre =  ReleaseTimeDomain(1);
params_releases.post = ReleaseTimeDomain(2);
params_releases.binwidth = 20;


t_releases = PSTHOut.Releases.Time;
[psth_releases_all, ts_releases_all, trialspxmat_releases_all, tspkmat_releases_all,  t_correct_releases_all,...
    ind] = Spikes.jpsth(r.Units.SpikeTimes(ku).timings,...
    t_releases, params_releases);
psth_releases_all = smoothdata (psth_releases_all, 'gaussian', 5);
PSTH.ReleasesAll =  {psth_releases_all, ts_releases_all, trialspxmat_releases_all, tspkmat_releases_all,  t_correct_releases_all};
PSTH.ReleasesLabels = {'PSTH', 'tPSTH', 'SpikeMat', 'tSpikeMat', 'tEvents'};

psth_release_correct=psth_releases_all;
ts_release=ts_releases_all;
trialspxmat_release=trialspxmat_releases_all;
tspkmat_release=tspkmat_releases_all;
t_releases_correct = t_correct_releases_all;
% PSTH.Releases = PSTH.ReleasesAll;
...
move_time =  PSTHOut.Pokes.RewardPoke.Move_Time;
    ...
    [psth_reward_pokes, ts_reward_pokes, trialspxmat_reward_pokes, tspkmat_reward_pokes,...
        t_reward_pokes, ind] = Spikes.jpsth(r.Units.SpikeTimes(ku).timings, t_reward_pokes, params);
    psth_reward_pokes = smoothdata (psth_reward_pokes, 'gaussian', 5);
    move_time = move_time(ind);
    PSTH.RewardPokes =  {psth_reward_pokes, ts_reward_pokes, trialspxmat_reward_pokes,...
        tspkmat_reward_pokes, t_reward_pokes,move_time};
...
% RT_triggers = PSTHOut.Triggers.RT{nFPs+1};
% FP_triggers = PSTHOut.Triggers.FP{end};
[psth_trigger, ts_trigger, trialspxmat_trigger, tspkmat_trigger, t_triggers,...
    ind] = Spikes.jpsth(r.Units.SpikeTimes(ku).timings,...
    t_triggers, params);
...
PSTH.Triggers =  {psth_trigger, ts_trigger, trialspxmat_trigger,...
    tspkmat_trigger, t_triggers};
PSTH.TriggerLabels = {'PSTH', 'tPSTH', 'SpikeMat', 'tSpikeMat', 'tEvents'};
% for i =1:nFPs
    % t_triggers_correct = PSTHOut.Triggers.Time;
    % RT_triggers_correct = PSTHOut.Triggers.RT;
...
psth_trigger_correct = psth_trigger;
ts_trigger_correct = ts_trigger;
trialspxmat_trigger_correct = trialspxmat_trigger;
tspkmat_trigger_correct = tspkmat_trigger;
t_triggers_correct = t_triggers;
...
FRMax = 10;
    ...
    hplot_press = plot(ts_press_all, psth_presses_all, 'color', FP_cols(3, :),  'linewidth', 1.5);
    FRMax = max([FRMax max(psth_presses_all)]);
...
ntrials_press = size(trialspxmat_press_all, 2);

axes('unit', 'centimeters', 'position', [1.25 yshift_row2 6 ntrials_press*rasterheight],...
    'nextplot', 'add',...
    'xlim', [-PressTimeDomain(1) PressTimeDomain(2)], 'ylim', [-ntrials_press 1], 'box', 'on');
yshift_row3 = yshift_row2+ntrials_press*rasterheight+0.5;
% Paint the foreperiod
k=0;

    ap_mat = trialspxmat_press;
    t_mat = tspkmat_press;
    % rt = rt_presses_sorted;
    xx_all = [];
    yy_all = [];
    xxrt_all = [];
    yyrt_all = [];
    x_portin = [];
    y_portin = [];
    for i =1:ntrials_press
        % irt = rt(i); % time from foreperiod to release
        xx = t_mat(ap_mat(:, i)==1);
        yy1 = [0 0.8]-k;
        yy2 = [0 1]-k;
        
        xxrt = t_releases_correct(i)-t_presses_correct(i);
    ...
    line(xx_all, yy_all, 'color', FP_cols(3, :), 'linewidth', 1);
...
if size(r.Units.SpikeNotes, 2) == 4
    ...
    uicontrol('style', 'text', 'units', 'centimeters', 'position', [1 yshift_row4+2 6 1.2],...
...
if ~isfield(r.Units, 'SpikeNotesColumn5')
    str_this = datestr(r.Meta(1).DateTime, 'yyyymmdd');
else
    anm_brain = [r.Meta(1).Subject, '(', r.Units.SpikeNotesColumn5{r.Units.SpikeNotes(ku, 5)},')'];
    str_this =  [anm_brain, ' ',  datestr(r.Meta(1).DateTime, 'yyyymmdd');];
end

uicontrol('style', 'text', 'units', 'centimeters', 'position', [1 yshift_row4+3.2 6 0.5],...
    'string', str_this, 'BackgroundColor','w',...
    'fontsize', 10, 'fontweight', 'bold',  'FontName','Dejavu Sans');


fig_height = yshift_row4+2;
...
yshift_row2 = yshift_row1+2+0.25;
    ...
    hplot_release = plot(ts_release, psth_release_correct, 'color', FP_cols(3, :),  'linewidth', 1.5);
    FRMax = max([FRMax max(psth_release_correct)]);
%     disp(FRMax)
...
% Make raster plot
% Plot spike raster of correct trials (all FPs)
ntrials_release = size(trialspxmat_release, 2);


axes('unit', 'centimeters', 'position', [8.25 yshift_row2 width ntrials_release*rasterheight],...
    'nextplot', 'add',...
    'xlim', [-ReleaseTimeDomain(1) ReleaseTimeDomain(2)], 'ylim', [-ntrials_release 1], 'box', 'on');
yshift_row4 = yshift_row2+ntrials_release*rasterheight+0.5;
% Paint the foreperiod
n_start = 1;
k=0;

    ap_mat = trialspxmat_release;
    t_mat = tspkmat_release;
    % rt = rt_releases_sorted;
    % mFP = MixedFPs(m);
    xx_all = [];
    yy_all = [];
    xxrt_all = [];
    ...
    x_portin = [];
    y_portin = [];
    for i =1:ntrials_release
        % irt = rt(i); % time from foreperiod to release
        xx = t_mat(ap_mat(:, i)==1);
        yy1 = [0 0.8]-k;
        yy2 = [0 1]-k;
        xxrt = t_presses_correct(i)-t_releases_correct(i);
        % paint foreperiod
        ...
        % port access time
        itrelease =t_releases_correct(i);
...
% Add information
uicontrol('Style','text','Units','centimeters','Position',[7.75 yshift_row4 width+1 1],...
...
ha_poke =  axes('unit', 'centimeters', 'position', [col3 yshift_row1 width 2], 'nextplot', 'add', ...
    ...
    plot(ts_reward_pokes, psth_reward_pokes, 'color', FP_cols(3, :), 'linewidth', 1.5);
    FRMax = max([FRMax max(psth_reward_pokes)]);
%     disp(FRMax)
...
% Make raster plot
% Plot spike raster of correct trials (all FPs)
ntrials_rewardpoke =  size(trialspxmat_reward_pokes, 2);
...
k = 0;
    ...
    ap_mat = trialspxmat_reward_pokes;
    t_mat = tspkmat_reward_pokes;
    move=move_time;
    % mFP = MixedFPs(m);
    xx_all = [];
    yy_all = [];
...
ha_trigger =  axes('unit', 'centimeters', 'position', [col4 yshift_row1 width 2], 'nextplot', 'add', ...
    ...
    plot(ts_trigger_correct, psth_trigger_correct, 'color', FP_cols(3, :), 'linewidth', 1.5);
    FRMax = max([FRMax max(psth_trigger_correct)]);
%     disp(FRMax)
...
ntrials_trigger = size(trialspxmat_trigger_correct, 2);
...
k=0;
    ...
    ap_mat = trialspxmat_trigger_correct;
    t_mat = tspkmat_trigger_correct;
...
% Add information
uicontrol('Style','text','Units','centimeters','Position',[col4-0.5  yshift_row3 5 1.2],...
    'string', 'E. Trigger-related activity', ...
    'FontName','Dejavu Sans', 'fontweight', 'bold','fontsize', 10,'BackgroundColor',[1 1 1],'ForegroundColor', 'k', ...
    'HorizontalAlignment','Left');
yshift_row4=yshift_row3+1.2+1.2;
...
ha0=axes('unit', 'centimeters', 'position', [col5 yshift_row4 1.5 1.5], ...
...
ha00= axes('unit', 'centimeters', 'position', [19.5+1.5+1 yshift_row4 2 1.5], 'nextplot', 'add', 'xlim', [-25 25]);
if median(c)>1
...
yshift_row5 = yshift_row4+2;
% Plot all waveforms if it is a polytrode
if isfield(r.Units.SpikeTimes(ku), 'wave_mean')
    ha_wave_poly = axes('unit', 'centimeters', 'position', [col5 yshift_row5 4 3], 'nextplot', 'add');
    ...
    set(ha_wave_poly, 'xlim', [0 max_x], 'ylim', [-20  v_sep*(n_rows-1)+20]);    axis off
    axis tight

    yshift_row6 = yshift_row5+3;
else
    yshift_row6 = yshift_row5;
end

uicontrol('Style','text','Units','centimeters','Position',[19 yshift_row6 5 1.5],...
    'string', 'F. Spike waveform and autocorrelation', ...
    'FontName','Dejavu Sans', 'fontweight', 'bold','fontsize', 10,'BackgroundColor',[1 1 1],'ForegroundColor', 'k', ...
    'HorizontalAlignment','Left');
fig_height = max([fig_height, yshift_row6+2]);
%% add location
ha_loc =  axes('unit', 'centimeters', 'position', [25.5 yshift_row5 2 3], 'nextplot', 'add');
loxX = unique(r.Units.ChanMap.xcoords);
loc_x = r.Units.ChanMap.xcoords(ch);
loc_x_ind = find(loxX == loc_x);
loc_y = r.Units.ChanMap.ycoords(ch);
y_min = min(r.Units.ChanMap.ycoords);
y_max = max(r.Units.ChanMap.ycoords);
x_positions = [1, 2, 3, 4];

for i = 1:4
    plot([x_positions(i), x_positions(i)], [y_min, y_max], '-', 'LineWidth', 2, 'Color', [128, 128, 128]/255);
end
plot(loc_x_ind, loc_y, 'ro', 'MarkerFaceColor', 'r', 'MarkerSize', 8);
text(loc_x_ind + 0.1, loc_y, sprintf('y=%d um', loc_y), ...
    'VerticalAlignment', 'middle', 'FontSize', 8);
box on;
grid off;

set(gca, 'XTick', x_positions, 'XTickLabel', {'1', '2', '3', '4'}, 'XLim',[0.5 4.5]);
set(gca, 'YTick', linspace(y_min, y_max, 5), 'YLim',[y_min-100 y_max+100]); 
% xlabel('X Position');
ylabel('Y Coordinate (um)');
% title('Vertical Lines with Labeled Points');
%%
% change the height of the figure
set(hf, 'position', [2 2 29 fig_height])
toc;
...
if strcmpi(ToSave,'on')
    ...
    if isfield(r.Units, 'SpikeNotesColumn5')
        ...
        tosavename2= fullfile(thisFolder, [session '_Ch'  num2str(ch) '_Unit' num2str(unit_no) ]);
    end
