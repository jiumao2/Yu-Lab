function r = ExtractEventFrameSignal(r, ts, varargin)
...
% get essential psths
psthall = ExtractAllPSTH(r, 'tpre', 10*timestep*3, 'tpost', 10*timestep*3);  % 10 ms * 3 * timestep
...
switch events{2}
    case 'Press'
        %% press
        press_frameindex = cell(1, length(t_press_sort));
        
        for n =1:length(t_press_sort);
            
            if n>1
                nstart = nstart + length(t_seg{n-1});  % frames of previous video segments
            end;
            
            t_press_n = t_press_sort{n};
            n_pressn = length(t_press_n);
            ...
            for i =1:n_pressn
                ...
                ind_frame_postpress_all         =          find(t_frameon>t_press_n(i), 1, 'first');
                ind_frame_postpress               =          ind_frame_postpress_all-nstart+voff(n); % this is the frame position in that movie clip
                
                frames_to_extract = [-3:3]*timestep+ind_frame_postpress;  % voff is the offset between frame and frame signal.
                indseq =[-3 : 3]*timestep;
                press_frameindex{n}(i) = ind_frame_postpress;
                
                tframes = t_frameon([-3:3]*timestep+ind_frame_postpress_all); % this is the time of plotted frames, session time, not segment time
                ...
                uicontrol('style', 'text', 'unit', 'centimeters', 'position', [0.1 10.5+6, 10, 0.5],...
                    'string', [ts.sideviews{n} '_press' '_' num2str(i), ' / t=' num2str(round(t_press_n(i))) 'ms'], 'fontsize', 10)
                ...
                psthall2 = [];
                for nc=1:Ncell
                    psthall2(nc, :) =  psthall(nc).press_correct{1};
                    plot(psthall(nc).t_press_correct{1}, psthall(nc).press_correct{1}, 'color', colorcodes(spkout.spk_chs(nc) == ch_included, :), 'linewidth', 1)
                end;
                ...
                tosavename2= fullfile(thisFolder, strrep([ts.sideviews{n} '_press' '_' num2str(i)], '.', '_'));
                ...
                if makemov
                    ...
                    tframes_to_extract = t_frameon([-3*timestep:3*timestep]+ind_frame_postpress_all); % this is the time of plotted frames, session time, not segment time
                    tic
                    [img_seq, headerInfo] = ReadJpegSEQ(ts.sideviews{n},[frames_to_extract(1) frames_to_extract(end)]); % ica is image cell array
                    toc
...
%                         FP_ni = FPs_sort{n}(i);
                    ...
                    moviename = strrep([ts.sideviews{n} '_press' '_' num2str(i)], '.', '_');
                    
                    writerObj = VideoWriter([moviename '.avi']);
                    writerObj.FrameRate = 10; % this is 5 x slower
