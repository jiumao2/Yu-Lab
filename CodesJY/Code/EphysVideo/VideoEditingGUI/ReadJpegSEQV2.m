function [ImageCellArray, headerInfo] = ReadJpegSEQ(fileName,frames,path_jpg)
% -------------------------------------------------------------------------
...
%             end
            ...
            tempName=[path_jpg,'\',fileName(end-24:end-8),'.',num2str(i+framecopy-1,'%06d'),'.jpg'];
            ...
            % timestampName=[path_jpg,'\', fileName(end-24:end-8),'.txt'];
            time([end-6,end-9,end-12]) = '_';
            timestampName = [path_jpg,'\',num2str(i+framecopy-1,'%06d'),'-',time,'.txt'];
            timestampFile = fopen(timestampName,'a');
            % fprintf(timestampFile,[time,'\n']);
