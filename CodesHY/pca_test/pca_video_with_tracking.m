...
p_threshold = r.VideoInfos(1).Tracking.p_threshold;
index_nose = find(contains(r.VideoInfos(1).Tracking.BodyParts,bodypart));
x_nose_all_raw = zeros(length(r.VideoInfos),length(r.VideoInfos(1).VideoFrameTime));
y_nose_all_raw = zeros(length(r.VideoInfos),length(r.VideoInfos(1).VideoFrameTime));
p_nose_all_raw = zeros(length(r.VideoInfos),length(r.VideoInfos(1).VideoFrameTime));
for k = 1:length(r.VideoInfos)
    x_nose_all_raw(k,:) = r.VideoInfos(k).Tracking.Coordinates_x{index_nose};
    y_nose_all_raw(k,:) = r.VideoInfos(k).Tracking.Coordinates_y{index_nose};
    p_nose_all_raw(k,:) = r.VideoInfos(k).Tracking.Coordinates_p{index_nose};
