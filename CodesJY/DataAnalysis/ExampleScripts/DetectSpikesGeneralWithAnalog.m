...
AllChs = [1:Nch 33:39];
EphysChs =[1:Nch];
LiveChs = setdiff(EphysChs, 9); % all live channels for 32-ch arrays (one can choose only a subset of these channels if not all channels have good data)
AllChs = [LiveChs 33:39];

[~, ind_live]=intersect(AllChs, LiveChs);
Fs = 30000; % this is the sampling rate

RMS_Chs = zeros(1, Nch);  % we use this to select "good" chs which will be used later to construct a common avg. 
...
if pardata ==1
    ...
    savefile = ['OptoStim' , '.mat']; % this is the force sensor data
    save(savefile, 'data', 'index');
...
ChsGood = EphysChs;  % these are the index of good Chs
