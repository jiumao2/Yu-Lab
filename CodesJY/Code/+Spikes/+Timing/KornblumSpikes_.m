...
t_correctpresses_cue        =           t_presses(ind_correct_cue);
t_correctpresses_uncue    =           t_presses(ind_correct_uncue);

t_correctreleases_cue           =       t_releases(ind_correct_cue);
t_correctreleases_uncue       =       t_releases(ind_correct_uncue);

FPs_correctpresses_cue   =           rb.Foreperiods(ind_correct_cue)'; 
FPs_correctpresses_uncue   =       rb.Foreperiods(ind_correct_uncue)'; 

rt_correct_cue          =        t_correctreleases_cue - t_correctpresses_cue - FPs_correctpresses_cue;
rt_correct_uncue      =        t_correctreleases_uncue - t_correctpresses_uncue - FPs_correctpresses_uncue;
...
t_correctreleases = [t_correctreleases_cue; t_correctreleases_uncue];
...
for i = 1:length(t_triggers)    
    ...
    [it_release, ~] = min(abs(t_correctpresses_cue-it_trigger));
    if it_release<2000
...
t_prematurepresses_cue      =            t_presses(ind_premature_cue);
t_prematurepresses_uncue  =            t_presses(ind_premature_uncue);
t_prematurereleases_cue         =       t_releases(ind_premature_cue);
t_prematurereleases_uncue     =       t_releases(ind_premature_uncue);  
...
t_latepresses_cue      =            t_presses(ind_late_cue);
t_latepresses_uncue  =            t_presses(ind_late_uncue);
t_latereleases_cue         =       t_releases(ind_late_cue);
t_latereleases_uncue     =       t_releases(ind_late_uncue);  
