...
name = 'Chen';
blocks                                             = { '001','002','003'}; % these are different sessions
...
if block_num>1
    ...
    for i_file =1:length(ns6_files)
        if i_file ==1
            openNSx(ns6_files(i_file).name, 'read', 'report')
            NS6all= NS6;
        else
            openNSx(ns6_files(i_file).name, 'read', 'report')
            NS6all(i_file)= NS6;
...
for i                                              = 1:size(r.Units.SpikeNotes, 1)
    ...
    if block_num>1
        ...
        id_temp = find(spk_id.cluster_class(:, 1)==cluster_id);
        r.Units.SpikeTimes(i).wave=[r.Units.SpikeTimes(i).wave;  spk_id.spikes(id_temp(id_ib), :)];
    else
