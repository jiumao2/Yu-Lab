function whiskingepochstim2(T, dataout, tdelay, vrange, md, whiskamps, th, showspikes, notouch, whisk_th, touchwhisker, whisk_nonwhisk,  tosave, wid);
global th
...
for i=1:length(selected_trialnums)
    ...
    if notouch==2
        ...
        if isempty(touchwhisker)
            ...
            r=T.detectFirstContacts(touchwhisker, selected_trialnums(i), whisk_th);
            if ~isempty(cell2mat(r))
                t_contact=cell2mat(r);
        ...
        t_contact=t_contact-0.025; % subtract 25ms for more conservative estimate
    end;
    twhisk=[0:1/1000:4.999];
    [wpos, tw]=T.get_whisker_position(dataout.whiskid, selected_trialnums(i));
...
hf_avg=figure;
set(hf_avg, 'units', 'centimeters', 'position', [4 4 15 10], 'paperpositionmode', 'auto')
...
hf1=figure;
set(gcf, 'units', 'centimeters', 'position', [4 4 20 14], 'paperpositionmode', 'auto');
...
for i=1:length(whiskamps)-1
    ...
    % nostim_vmepochs
    if length(ind_nostim)>5
        [vm_whiskamp_nostim_avg(i, :), vm_whiskamp_nostim_all{i}]=analyze_vmepoch(nostim_vmepochs(ind_nostim));
    ...
    if length(ind_stim)>5
        [vm_whiskamp_stim_avg(i, :), vm_whiskamp_stim_all{i}]=analyze_vmepoch(stim_vmepochs(ind_stim));
...
if tosave
    ...
    whiskingvmout.vm.nostim_nonwhisking_avg=analyze_vmepoch(vmnostim_nonwhisking_epochs);
    whiskingvmout.vm.nostim_nonwhisking_epochs=vmnostim_nonwhisking_epochs;
    whiskingvmout.vm.nostim_whisking_avg=analyze_vmepoch(vmnostim_whisking_epochs);
    whiskingvmout.vm.nostim_whisking_epochs=vmnostim_whisking_epochs;
    
    whiskingvmout.vm.stim_nonwhisking_avg=analyze_vmepoch(vmstim_nonwhisking_epochs);
    whiskingvmout.vm.stim_nonwhisking_epochs=vmstim_nonwhisking_epochs;
    
    whiskingvmout.vm.stim_whisking_avg=analyze_vmepoch(vmstim_whisking_epochs);
...
%     
    ...
    saveas(hf1, ['vmspk_whisk_nonwhisk' '_wid'  num2str(dataout.whiskid) '_' num2str(notouch)],'tif')
%     saveas(hf2, ['vmspk_whiskamps' '_wid' num2str(dataout.whiskid) '_' num2str(notouch)],'tif')
    saveas(hf1, ['vmspk_whisk_nonwhisk' '_wid' num2str(dataout.whiskid) '_' num2str(notouch)],'fig')
%     saveas(hf2, ['vmspk_whiskamps' '_wid' num2str(dataout.whiskid) '_' num2str(notouch)],'fig')
    saveas(hf1, ['vmspk_whisk_nonwhisk' '_wid' num2str(dataout.whiskid) '_' num2str(notouch)],'pdf')
%     saveas(hf1, ['vmspk_whisk_nonwhisk' '_wid' num2str(dataout.whiskid) '_' num2str(notouch)],'pdf')
