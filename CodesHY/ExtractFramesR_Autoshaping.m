function r = ExtractFramesR_Autoshaping(r, ts, varargin)
% requires "Video" field in r
    ...
    ind_trigger = find(strcmp(r.Behavior.Labels, 'Trigger'));
    ind_valve = find(strcmp(r.Behavior.Labels, 'ValveOnset'));
    t_trigger = r.Behavior.EventTimings(r.Behavior.EventMarkers==ind_trigger);
    t_valve = r.Behavior.EventTimings(r.Behavior.EventMarkers==ind_valve);
    FPs = NaN(size(t_trigger));
    RTs = NaN(size(t_trigger));
    for i = 1:length(t_trigger)
        idx_valve = find(t_valve > t_trigger(i), 1, 'first');
        if ~isempty(idx_valve)
            RTs(i) = t_valve(idx_valve) - t_trigger(i);
        end
    end
