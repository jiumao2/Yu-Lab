function PSTHOut = KornblumRampingPressDur(r, ind, varargin)

% Jianing Yu 2022.10.7
...
t_correctpresses = t_presses([reshape(rb.CorrectIndex, 1, []) reshape(rb.PrematureIndex, 1, [])]); % also takes premature presses
FPs_correctpresses = rb.Foreperiods([reshape(rb.CorrectIndex, 1, []) reshape(rb.PrematureIndex, 1, [])]);
...
t_correctreleases = t_releases([reshape(rb.CorrectIndex, 1, []) reshape(rb.PrematureIndex, 1, [])]); 
...
[PSTHUncue, tsUncue, trialspxmatUncue, tspkmatUncue, t_correctpressesUncue] = jpsth(r.Units.SpikeTimes(ku).timings, t_correctpresses_uncue, params_press);
PSTHUncue = smoothdata (PSTHUncue, 'gaussian', 5);
